...
import { queryInitiatedProcess, querySearchListForInitiatedProcess } from '@/api/bpm/bpm'
...
export default {
  ...
  watch: {
    $route: {
      handler: function (route) {
        ...
        if (params.isGetList && route.name === 'MyInitiatedProcess') {
  ...
  methods: {
    ...
    // 下拉搜索列表
    getSearchData() {
      querySearchListForInitiatedProcess({}).then((res) => {
        const { moduleKeyList, handleUserList } = res.data
        this.$set(this.searchData[1], 'selectData', moduleKeyList)
        this.$set(this.searchData[2], 'selectData', handleUserList)
      })
    },
    showCancel(row) {
      const { moduleKey } = row
    ...
    actionHandle(row, type, index) {
      ...
      if (type === 'toDo' || type === 'history') {
        const fromSrcPath = encodeURIComponent('/bpm/processManagement/myInitiatedProcess')

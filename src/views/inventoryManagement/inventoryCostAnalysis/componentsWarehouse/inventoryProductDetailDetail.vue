<template>
  ...
  >
    ...
    <el-row type="flex" justify="space-between" class="mt20">
      <el-col>
        <el-button type="primary" size="mini" @click="StockOutStockInDetailsSummary">{{
          $t('PURCHASE.StockOutStockInDetailsSummary')
        }}</el-button>
      </el-col>
      <el-col>
        <right-toolbar
    ...
    <!-- 明细汇总 -->
    <stockOutStockInDetailsSummaryDlg ref="stockOutStockInDetailsSummaryDlg" />
    <!-- 明细 -->
    <stockOutStockInDetailsDlg ref="stockOutStockInDetailsDlg" />
  </el-dialog>
</template>

<script>
import pageMixin from '@/mixins/tableMinx'
import { queryInventoryCostProductWarehouseDocumentLogisticsList } from '@/api/inventoryManagement/distributionByWarehouse'
import locale from '@/views/inventoryManagement/lang/index'
import productDetailChartDlg from './productDetailChartDlg'
import stockOutStockInDetailsSummaryDlg from './stockOutStockInDetailsSummaryDlg'
import stockOutStockInDetailsDlg from './stockOutStockInDetailsDlg'

export default {
  mixins: [pageMixin],
  components: {
    productDetailChartDlg,
    stockOutStockInDetailsSummaryDlg,
    stockOutStockInDetailsDlg
  },
  data() {
    const vm = this
    return {
      saveKey: '132',
      savePath: 'inventoryProductDetailDetail',
      loading: false,
      // 显示搜索条件
      showSearch: true,
      // 总条数
  ...
  methods: {
    ...
    getList() {
      ...
      queryInventoryCostProductWarehouseDocumentLogisticsList(param)
        .then((response) => {
    ...
    StockOutStockInDetailsSummary() {
      this.$refs.stockOutStockInDetailsSummaryDlg.handleOpen(this.form)
    },
    handleDetail(row) {
      this.$refs.stockOutStockInDetailsDlg.handleOpen(row)
    },
    /** 搜索 */
    handleSearchForm() {
      this.queryParams.pageNum = 1

<template>
  ...
  >
    <div class="right-title">
      ...
      <span class="mr20" :title="showUomLabel(form.basicUom)">
        {{ $t('PRODUCT.uom1') }}：{{ form.basicUom }}
      </span>
...
import { queryProductInventoryCostSettingsMsg } from '@/api/inventoryManagement/distributionByWarehouse'
...
export default {
  ...
  methods: {
    ...
    openChart() {
      this.$refs.chartDlg.openChart(this.productId, this.warehouseId)
    },
    getList() {
      const vm = this
      this.loading = true
      ...
      queryProductInventoryCostSettingsMsg(param)
        .then((response) => {

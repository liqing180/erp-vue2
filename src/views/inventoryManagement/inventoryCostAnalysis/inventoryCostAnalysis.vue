...
export default {
  ...
  data() {
    ...
    return {
      ...
      importOptions: {
        // 下载模板
        downFileUrl: '/inventory/inventoryCostProduct/exportImportTemplate',
        // 上传校验
        uploadUrl:
          process.env.VUE_APP_BASE_API + '/inventory/inventoryCostProduct/checkImportTemplateData',
        // 提交导入
        importUrl: '/inventory/inventoryCostProduct/importTemplateData',
        // 下载文件名
        fileName: 'Inventory Cost Analysis'
      }
    }
  },
  watch: {},
      ...
      queryInventoryCostProductList(param)
        .then((response) => {
          ...
          if (rows.length > 0 && response.summary) {
            ...
            console.log(summary, '===============420')
            rows.push(summary)
          }
          console.log(rows, '====================422')
          this.tableList = rows
          this.loading = false

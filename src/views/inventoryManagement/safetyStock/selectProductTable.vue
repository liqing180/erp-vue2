...
import { querySafetyStockCanSelectProductList } from '@/api/inventoryManagement/safetyStock'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.loading = true
      querySafetyStockCanSelectProductList(param)
        .then((response) => {
          this.loading = false
    ...
    submit() {
      ...
      /* dealAddProductMsg({
        ivtProductList: this.selectList,
        warehouseId: this.warehouseId
      })
        .then((res) => {
          const data = res.data || {}
          this.$emit('onSuccess', data.ivtProductList || [])
          this.visible = false
          setTimeout(() => {
            this.btnLoading = false
          }, 2000)

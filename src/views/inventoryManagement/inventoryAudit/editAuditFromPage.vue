<template>
  <FormPageLayoutTabs v-loading="submitLoading">
    <template v-slot:btn>
      ...
      <div v-if="comeFrom === '2'" class="bpm-page-menu-title">{{ $t('menu.inventoryAudit') }}</div>
    </template>
        ...
        <el-collapse v-model="activeNames">
              ...
              >
                ...
                <el-row>
                  ...
                  <el-col v-if="form.inventoryAuditMethod === '1'" :span="8">
                    <el-form-item :label="$t('INVENTORY.criteriaRange')">
                      <el-input-number
                        style="width: 100%"
                  ...
                  <el-col :span="8" v-else>
                    <el-form-item :label="`${$t('INVENTORY.scope')}`">
                      <el-input
                        v-model="form.warehouseStructureNames"
                        :title="form.warehouseStructureNames"
                        disabled
                      ></el-input>
                    </el-form-item>
                  </el-col>
                ...
                <el-row>
                  <el-col :span="8" v-if="form.inventoryAuditMethod !== '1'">
                    <el-form-item :label="`${$t('INVENTORY.productCategory')}`">
                      <el-input
                        v-model="form.productCategoryNames"
          ...
          <div class="form-card mt10 collapse-item-content-pt0">
            <el-collapse-item name="2">
              ...
              <div>
                <UserTable ref="UserTable" :formData="form" :comDisFrom="true" />
              </div>
            </el-collapse-item>
          </div>
          <div class="form-card mt10 collapse-item-content-pt0">
...
export default {
  ...
  computed: {
    ...
    startAuditAuth() {
      if (!this.checkPermi(['inventoryManagement:inventoryAudit:startAudit'])) {
        return false
      }
      let show = false
      if (
    ...
    endAuditAuth() {
      if (!this.checkPermi(['inventoryManagement:inventoryAudit:endAudit'])) {
        return false
      }
      let show = false
      if (
  ...
  methods: {
    ...
    handleEndAudit() {
      const validProduct = this.validProductList(this.form.detailList)
      if (!validProduct) {

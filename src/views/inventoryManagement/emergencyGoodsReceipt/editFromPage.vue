  ...
  <FormPageLayout v-loading="submitLoading">
    <template v-slot:btn>
      ...
      <el-button
        type="primary"
        size="mini"
        v-if="extendedCancelledBtnShow"
        @click="handleExtendedCancel"
        >{{ $t('uiBtn.cancel') }}
      </el-button>
      <el-button
        type="primary"
              ...
              >
                ...
                <el-row>
                  <el-col :span="8">
                    ...
                    >
                      <ComparisonInput
                        v-if="isModified('deliveryOrderNo')"
                        :beforeValue="getBeforeValue('deliveryOrderNo')"
                        :afterValue="getAfterValue('deliveryOrderNo', form.deliveryOrderNo)"
                      />
                      <el-input v-else v-model="form.deliveryOrderNo" maxlength="50"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item
                  ...
                  <el-col :span="8">
                    <el-form-item :label="`${$t('PURCHASE.receivedDate')}`" prop="receivedDate">
                      <ComparisonInput
                        v-if="isModified('receivedDate')"
                        :beforeValue="getBeforeValue('receivedDate')"
                        :afterValue="getAfterValue('receivedDate', form.receivedDate)"
                      />
                      <el-date-picker
                        v-else
                  ...
                  <el-col :span="16">
                    ...
                    >
                      <!-- <SelectInput
                        ...
                        :disabled="form.emergencyGoodsStatus !== '1'"
                        class="form-wd"
                      /> -->
                    ...
                    <template slot-scope="scope">
                      <template v-if="item.prop === 'productName'">
                        ...
                        <selectExpenseItemName
                          ...
                          v-else-if="!comDisFrom && scope.row.isExpenseItem === '1'"
                        />
                        <template v-else>
                      ...
                      <template v-else-if="item.prop === 'receivedQty'">
                        <el-input-number
                          ...
                          v-if="!comDisFrom && scope.row.isExpenseItem === '1'"
                        />
                        <el-input-number
                          style="width: 98%"
    ...
    <ApprovedDialog
      :id="form.emergencyGoodsReceiptId"
      ...
      formDataKey="emergencyGoodsReceipt"
    />
    <RejectDialog
      :id="form.emergencyGoodsReceiptId"
      :taskId="taskId || form.taskId"
    ...
    />
    <RevisionComparisonDlg module-key="emergencyGoodsReceipt" ref="RevisionComparisonDlg" />
  </FormPageLayout>
</template>
...
import {
  ...
  approvedEmergencyGoodsReceipt,
  rejectedEmergencyGoodsReceipt,
  withdrawApproved,
  cancelledEGR,
  checkCanExtendedCancelled,
  extendedCancelledEGR,
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rjVisible: false,
      aplVisible: false,
      approvedUrl: approvedEmergencyGoodsReceipt,
      rejectedUrl: rejectedEmergencyGoodsReceipt,
      cancelledUrl: cancelledEGR,
  ...
  computed: {
    configCurColumn() {
      ...
      if (!this.rowId || ['4', '7'].indexOf(this.form.emergencyGoodsStatus) === -1) {
        arr = arr.filter((x) => x.prop !== 'stockInQty')
      }
    ...
    comDisFrom() {
      ...
      if (
        ...
        this.form.emergencyGoodsReceiptId &&
    ...
    submitBtnShow() {
      ...
      if (this.form.emergencyGoodsReceiptId && this.buttonAuthMsg.isCanUpdate === '1') {
        show = true
      }
      return show
    },
    ...
    withdrawApproveBtnShow() {
      if (!this.checkPermi(['inventoryManagement:emergencyGoodsReceipt:withdrawApprove'])) {
        return false
      }
      let show = false

      if (this.form.emergencyGoodsReceiptId && this.buttonAuthMsg.isCanWithdrawApproved === '1') {
        show = true
      }
      return show
    },
    cancelBtnShow() {
      if (!this.checkPermi(['inventoryManagement:emergencyGoodsReceipt:cancel'])) {
        return false
      }
      let show = false
      if (
    ...
    extendedCancelledBtnShow() {
      ...
      if (
        this.dataType !== '2' &&
        this.form.emergencyGoodsReceiptId &&
        this.buttonAuthMsg.isCanExtendedCancelled === '1'
      ) {
        show = true
      }
      return show
  ...
  methods: {
    revisionComparison() {
      this.$refs.RevisionComparisonDlg.handleOpen(this.form.emergencyGoodsReceiptId)
    },
      ...
      this.$modal
        ...
        .then(() => {
          return withdrawApproved({
            businessId: this.form.emergencyGoodsReceiptId,
    ...
    handleUpdate() {
      ...
      queryEmergencyGoodsReceiptById({
        ...
        this.queryZoneListByProductIdList()
    ...
    initVersionComparison(data, currentDetailList) {
      ...
      if (
        (updateMsg.beforeCommonFileListForDO || []).length ||
        (updateMsg.afterCommonFileListForDO || []).length
      ) {
        this.basicUpdateProps.push('attachmentDo')
      }

      this.detailListCur = cloneList(currentDetailList)
      this.commonFileListCur = cloneList(data.commonFileList)
      this.commonFileListForDOCur = cloneList(data.commonFileListForDO)
      ...
      const deletedDetailList = onlyDeleted(updateMsg.beforeDetailList).map((item, index) => {
        ...
        ;(item.serviceSurchargeList || []).forEach((child, childIndex) => {
          child.updateType = '3'
          child.rowTimeId = child.rowTimeId || `${item.rowTimeId}-${childIndex}`
        })
        return item
      })
      const detailList = this.isComparison
        ? [...cloneList(currentDetailList), ...deletedDetailList]
        : cloneList(currentDetailList)
      this.createTableList = detailList
      this.tableList = this.createTableList
      this.changeTableList()
      ...
      this.$nextTick(() => {
        ...
        this.$refs.uploadDORef &&
          this.$refs.uploadDORef.initFileList(
            this.isComparison
              ? [
                  ...cloneList(data.commonFileListForDO),
                  ...onlyDeleted(updateMsg.beforeCommonFileListForDO)
                ]
              : this.commonFileListForDOCur
          )
      })
    },
    getFormJson() {
    ...
    updateTable(list) {
      if (list.length > 0) {
        ...
        rows.forEach((p) => {
          ...
          if (p.isPhantom === '1' && !p.serviceSurchargeList && !p.emergencyGoodsReceiptDetailId) {
      ...
      if (valid1) {
        ...
        if (submitType !== 'save') {
          await queryEmergencyGoodsReceiptNoListByDeliveryOrderNo({
            ...
            emergencyGoodsReceiptId: param.emergencyGoodsReceiptId
          })
            .then((res) => {
              const data = res.data || []
              if (data.length > 0) {
    ...
    exitComparison() {
      ...
      this.$nextTick(() => {
        ...
        this.$refs.uploadDORef && this.$refs.uploadDORef.initFileList(this.commonFileListForDOCur)
      })
    },
    getBeforeValue(prop) {
      const item = this.basicUpdateMsgList.find((row) => row.name === prop) || {}
      if (prop === 'receivedDate') {
    ...
    aplSubmitSuccess() {
      const vm = this
      vm.$message.success(
        `${vm.$t('ui.approvedSuccess')}`.replace('$1', `[${vm.form.emergencyGoodsReceiptNo}]`)
      )
    ...
    rjSubmitSuccess() {
      const vm = this
      vm.$message.success(
        `${vm.$t('ui.rejectedSuccess')}`.replace('$1', `[${vm.form.emergencyGoodsReceiptNo}]`)
      )
      this.rjVisible = false
      vm.back('onRejectedSuccess')
    },
    handleCancel() {
      this.cancelledUrl = cancelledEGR
      this.$refs.FormCancelDialog.handleOpen()
    },
    handleExtendedCancel() {
      this.cancelledUrl = extendedCancelledEGR
      this.submitLoading = true
      checkCanExtendedCancelled({ businessId: this.form.emergencyGoodsReceiptId })
        .then((res) => {
          this.submitLoading = false
          this.$refs.FormCancelDialog.handleOpen()
        })
    ...
    cancelSubmitSuccess() {
      const vm = this
      vm.$message.success(
        `${vm.$t('ui.cancelledSuccess')}`.replace('$1', `[${vm.form.emergencyGoodsReceiptNo}]`)
      )

<template>
  <el-dialog
    v-dialogDrag
    v-dialogDragWidth
    :title="$t('menu.warehouseAssignment')"
    ...
    @close="close"
  >
    <el-form ref="form" :model="queryParams" :rules="rules" class="mr20">
      <el-row>
        <el-col :span="12">
          <el-form-item :label="$t('INVENTORY.warehouseName')" prop="warehouseId">
            <el-select
              size="small"
              style="width: 200px"
              @change="warehouseIdChange"
              v-model="queryParams.warehouseId"
            >
              <el-option
                v-for="child in warehouseList"
                :key="child.warehouseId"
                :label="child.warehouseName"
                :value="child.warehouseId"
              >
              </el-option>
            </el-select>
          </el-form-item>
...
import {
  queryAssignmentCanSelectProductList,
  queryActiveWarehouseList,
  save,
  queryAssignmentByWarehouseId
        ...
        warehouseId: [
          {
            ...
            message: this.$t('ui.reqMsg').replace('$1', this.$t('INVENTORY.warehouseName')),
            trigger: ['change', 'blur']
          }
        ]
      },
  ...
  methods: {
    ...
    warehouseIdChange(value) {
      if (this.queryParams.warehouseId) {
        this.loading = true
        queryAssignmentByWarehouseId({ warehouseId: this.queryParams.warehouseId })
          .then((res) => {
            this.assignedProductIdList = res.data || []
            this.selectList = this.selectList.filter((item) => {
              if (this.assignedProductIdList.includes(item.productId)) {
    ...
    getList() {
      ...
      this.loading = true
      queryAssignmentCanSelectProductList(param)
        .then((response) => {
          this.loading = false
    ...
    selectable(row, rowIndex) {
      if (this.assignedProductIdList.includes(row.productId)) {
        return false
      }
      return true // 不禁用
    },
    ...
    handleSelectionChange(selection, row) {
      ...
      for (let i = 0; i < vm.selectList.length; i++) {
        if (vm.selectList[i].productId === row.productId) {
    ...
    handleRowClick(row, column) {
      ...
      const index = this.selectList.findIndex((item) => item.productId === row.productId)
    ...
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.$modal
            .confirm(this.$t('INVENTORY.warehouseAssignmentSubmit'))
            .then(() => {
              return save({
                warehouseId: this.queryParams.warehouseId,
                ivtProducts: this.selectList
              })
            })
            .then(() => {
              this.$emit('onSuccess')
              this.$message.success(`${this.$t('INVENTORY.warehouseAssignmentSuccess')}`)
              this.visible = false
            })
        }
      })

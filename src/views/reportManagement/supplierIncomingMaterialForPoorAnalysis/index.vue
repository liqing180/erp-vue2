...
import {
  queryListForBusiness,
  queryImageForBusinessDefectiveRateTrend
...
export default {
  ...
  methods: {
    ...
    getList(query) {
      ...
      this.queryImageForBusinessDefectiveRateTrend()
    },
    queryImageForBusinessDefectiveRateTrend() {
      const { startDate, endDate } = this.queryParams
      queryImageForBusinessDefectiveRateTrend({ startDate, endDate }).then((res) => {
        this.$nextTick(() => {
    ...
    handleExport(data) {
      ...
      this.$importOrExportDownFile(
        '/purchase/report/incomingAnalysis/exportForBusiness',
        params,
        `${this.getFileNameDate('SupplierIncoming Material For Poor Analysis')}.xlsx`
      )
        .then((res) => {

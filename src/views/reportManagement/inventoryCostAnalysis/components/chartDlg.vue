...
import { queryImageForInventoryCost } from '@/api/reportManagement/inventoryCostAnalysis'
...
export default {
  ...
  methods: {
    openChart(row) {
      ...
      this.visible = true
      this.queryImageForInventoryCost()
    },
    queryImageForInventoryCost() {
      const { dateRange, productId } = this.form
      const params = {
        productId
      }
      ...
      queryImageForInventoryCost(params).then((response) => {
        this.$nextTick(() => {

    ...
    <div class="app-container" v-if="currentIndex === 0">
      ...
      <el-row :gutter="10">
        <el-col :span="1.5">
          <el-button
            ...
            @click="openExport"
            v-hasPermi="['reportManagement:supplierReturnSummary:export']"
      ...
      >
        <el-table-column
          ...
          align="center"
          v-if="checkPermi(['reportManagement:supplierReturnSummary:export'])"
        >
        </el-table-column>
      ...
      <div class="flexCen mt20">
        <div style="width: 50%; min-width: 500px">
          <PieChart ref="PieChart" :precision="0" :title="$t('PURCHASE.supplierProportion')" />
        </div>
      </div>
    </div>
...
export default {
  name: 'SupplierReturnSummary',
  dicts: [],
  mixins: [pageMixin],
  components: { productAnalysis, productDetail, PieChart },
  data() {
    const vm = this
    return {
      saveKey: '1',
      ...
      columns: [
        ...
        {
          prop: 'purchaseOrderCountForReturn',
          label: vm.$t('PURCHASE.returnOrderQTY'),
          minWidth: 160,
          visible: true,
          tooltip: true,
          padding: 50,
  ...
  methods: {
    ...
    queryImageForSupplier() {
      const { endDateSubmittedTime, startDateSubmittedTime } = this.queryParams
      queryImageForSupplier({ endDateSubmittedTime, startDateSubmittedTime }).then((res) => {
        const data = res.data || {}
        let supplierReturnRateImageList = data.supplierReturnRateImageList || []
        const totalPurchaseOrderCountForReturn = data.totalPurchaseOrderCountForReturn || ''
        supplierReturnRateImageList = supplierReturnRateImageList.map((x) => {
          return {
            value: x.purchaseOrderCountForReturn,
            name:
              (x.reportName || x.businessPartnerName) + ' ' + (x.purchaseOrderCountForReturn || ''),
            topLabel: this.$t('PURCHASE.supplierName'),
            showName: x.reportName || x.businessPartnerName,
            totalAmount: totalPurchaseOrderCountForReturn
          }
        })
        this.$nextTick(() => {
          this.$refs.PieChart && this.$refs.PieChart.initChart(supplierReturnRateImageList)
        })
      })
    },
    ...
    handleExport(data) {
      ...
      this.$importOrExportDownFile(
        '/purchase/report/supplierReturn/exportForSupplier',
        params,
        `${this.getFileNameDate('Supplier Return Summary')}.xlsx`
      )
        .then((res) => {

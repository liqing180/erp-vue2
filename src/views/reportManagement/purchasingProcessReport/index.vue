      ...
      <li class="row" v-for="(item, index) in tableList" :key="index">
        <div class="desc flexSb">
          <div class="flexStart flow1">
            <div class="flexStart mr20 flow1" v-show="item.businessPartnerName">
              <div class="icon"></div>
              <div class="flow1" :title="item.businessPartnerName">
                {{ item.businessPartnerName }}
              </div>
            </div>
            <div class="flexStart mr20 flow1" v-show="item.purchaseRequisiteNo">
              <div class="icon icon-1"></div>
              <div class="flow1" :title="item.purchaseRequisiteNo">
                <!-- <span>{{ $t('PURCHASE.purchaseRequisitionNo') }}：</span> -->
                <span
                  style="color: #0077ff; cursor: pointer"
                  @click="nav(item, 'purchaseRequisiteNo')"
                  >{{ item.purchaseRequisiteNo }}</span
                >
              </div>
            </div>
            <div class="flexStart flow1" v-show="item.purchaseOrderNo || item.purchaseOrderNo">
              ...
              <div
                ...
                "
              >
                <!-- <span>{{ $t('PURCHASE.purchaseOrderNo') }}：</span> -->
                <span
                  style="margin-right: 3px; color: #0077ff; cursor: pointer"
                  @click="nav(item, 'purchaseOrderNo')"
                  >{{ item.purchaseOrderNo }}</span
                >
                <span
                  >({{ $t('PURCHASE.purchaseAmount') }}：{{ item.currencySymbol }}
                  {{ $numberStr(item.totalAmount, 2) }} )</span
                >
              </div>
            </div>
          </div>
          <div class="flexStart flow1">
            ...
            <div class="flexStart flow1">
              ...
              <div
                class="flow1"
                :title="$t('ui.status') + '：' + item.purchaseProcessViewStatusShowStr"
              >
                <span>{{ $t('ui.status') }}：</span>
                <span :class="'status-' + item.purchaseProcessViewStatus">{{
                  item.purchaseProcessViewStatusShowStr
                }}</span>
              </div>
            </div>
          </div>
        ...
        <ul class="card flexStart">
          <li class="item" v-for="(child, childIndex) in item.viewStageList" :key="childIndex">
            <div class="child-title">{{ child.purchaseProcessViewStageTypeShowStr }}</div>
            <div class="step-card">
              <ul class="flexCen">
                <li
                ...
                >
                  <div class="flexStart" style="width: 100%">
                    <div
                      ...
                      :class="{
                        'line-active':
                          son.purchaseProcessViewNodeStatus !== '1' &&
                          son.purchaseProcessViewNodeStatus !== '4'
                      }"
                      :style="{ opacity: sonIndex === 0 ? '0' : '1' }"
                    ></div>
                    <div class="flexCCen fs-0">
                      <img
                        src="./images/icon-8.png"
                        alt=""
                        class="step-icon"
                        v-if="son.purchaseProcessViewNodeStatus === '4'"
                      />
                      <img
                        src="./images/icon-5.png"
                        alt=""
                        class="step-icon"
                        v-else-if="son.purchaseProcessViewNodeStatus !== '1'"
                      />
                  ...
                  <div
                    ...
                    style="width: 100%"
                    :title="son.purchaseProcessViewNodeTypeShowStr"
                  >
                    {{ son.purchaseProcessViewNodeTypeShowStrAbbreviation }}
                  </div>
                </li>
              </ul>
...
import {
  queryCountList,
  queryPurchaseProcessViewList,
  querySearchList
} from '@/api/reportManagement/purchasingProcessReport'
export default {
  name: 'PurchasingProcessReport',
  dicts: [],
  data() {
    const vm = this
    return {
      ...
      searchData: [
        {
          name: 'condition',
          type: 'InputEle',
          placeholder: `${this.$t('PURCHASE.purchaseRequisiteNo')} / ${this.$t(
      ...
      tabs: [
        ...
        { label: this.$t('PURCHASE.pr'), value: '1', num: undefined },
        // { label: this.$t('PURCHASE.inquiry'), value: '2', num: undefined },
        // { label: this.$t('PURCHASE.inQuotation'), value: '3', num: undefined },
        { label: this.$t('PURCHASE.rfqpq'), value: '7', num: undefined },
        { label: this.$t('PURCHASE.purchasing'), value: '4', num: undefined },
        { label: this.$t('PURCHASE.partDelivered'), value: '5', num: undefined },
        { label: this.$t('PURCHASE.completed'), value: '6', num: undefined }
      ],
      activeName: '0',
      moduleList: [
        this.$t('PURCHASE.purchaseRequisition1'),
        this.$t('PURCHASE.enquiry'),
        this.$t('PURCHASE.quotation'),
        this.$t('PURCHASE.purchase'),
        this.$t('PURCHASE.inbound')
      ],
      timer: null,
      navLoading: false
    }
  ...
  methods: {
    ...
    handleClick(value) {
      // if (this.activeName === value) return
    ...
    getList(query) {
      ...
      this.loading = true
      queryPurchaseProcessViewList(param)
        .then((response) => {

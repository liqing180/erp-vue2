        ...
        >
          ...
          >
            <template slot-scope="scope">
              ...
              <template v-else-if="item.prop === 'warehouseUnqualifiedAmount'">
                {{ $numberStr(scope.row[item.prop], 2) }}
              </template>
...
<script>
import {
  queryListForWarehouseProductOrder,
  querySearchListForWarehouseProductOrder
...
export default {
  ...
  methods: {
    getList(query) {
      ...
      queryListForWarehouseProductOrder(param)
        .then((response) => {
    ...
    querySearchList() {
      ...
      querySearchListForWarehouseProductOrder({ businessPartnerId, productId }).then((res) => {
    ...
    handleExport(data) {
      ...
      this.$importOrExportDownFile(
        '/purchase/report/incomingAnalysis/exportForWarehouseProductOrder',
        params,
        `${this.getFileNameDate('Incoming Analysis')}.xlsx`
      )
        .then((res) => {

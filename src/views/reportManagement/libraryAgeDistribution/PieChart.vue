        ...
        tooltip: {
          ...
          formatter: function (params) {
            ...
            const res = `
              <table>
                <tr>
                  <td style="text-align: right;">${vm.$t('PURCHASE.stockTotal')} : </td>
                  <td>${vm.$numberStr(result.qtyOnHand, 3)}</td>
                </tr>
                <tr>
                  <td style="text-align: right;">${vm
                    .$t('PURCHASE.daysInventoryQty')
                    .replace('$1', result.days)} : </td>
                  <td>${vm.$numberStr(result.inventoryQty, 3)}</td>
                </tr>
                <tr>
                  <td style="text-align: right;">${vm.$t('PURCHASE.inventoryCost1')} : </td>
                  <td>${vm.$numberStr(result.inventoryAmount, 2)}</td>
                </tr>
                <tr>
                  <td style="text-align: right;">${vm
                    .$t('PURCHASE.daysInventoryAmount')
                    .replace('$1', result.days)} : </td>

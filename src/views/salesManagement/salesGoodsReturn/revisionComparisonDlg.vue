...
export default {
  ...
  computed: {
    ...
    comIsRejectedCreated() {
      // 销售退货单创建类型.1退货2拒收"
      return this.salesGoodsReturnCreateType === '2'
    },
    comVisibleColumn() {
      const arr = [...this.productColumns]
  ...
  methods: {
    ...
    async handleOpen(id) {
      ...
      queryUpdateMsgSalesGoodsReturn({ salesGoodsReturnId: id }).then((res) => {
        ...
        this.salesGoodsReturnCreateType = results.salesGoodsReturnCreateType
        this.visible = true
    ...
    handleList(list) {
      ...
      return list.map((item) => {
        if (item.replenishType === '1') {
          item.replenishType = vm.$t('SALES.warehouse')
        }
        if (item.replenishType === '2') {
          item.replenishType = vm.$t('SALES.dropShipping1')
        }

<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <template v-if="item.prop === 'salesOrderNo'">
            <DocumentNoLink
          ...
          <template v-else-if="item.prop === 'rmaTypeShowStr'">
            <el-tooltip placement="top" effect="light">
              <div slot="content">
                <div v-if="scope.row.salesGoodsReturnProductList.length > 0">
                  <table>
                    <tr>
                      <th style="padding: 5px 10px">{{ $t('SALES.RMAType') }}</th>
                      <th style="padding: 5px 10px">{{ $t('SALES.productName') }}</th>
                    </tr>
                    <tr
                      style="border-bottom: 1px solid #efefef"
                      v-for="(item, index) in scope.row.salesGoodsReturnProductList"
                      :key="index"
                    >
                      <td style="padding: 10px">{{ item.rmaTypeShowStr }}</td>
...
export default {
  ...
  computed: {
    addAuth() {
      return this.checkPermi(['salesManagement:salesGoodsReturn:add'])
    },

    exportAuth() {
      return this.checkPermi(['salesManagement:salesGoodsReturn:export'])
    },
    curColumns() {
  ...
  async created() {
    ...
    const a = this.MyDictDataClass.getDictFn('sales_goods_return_status')
    ...
    this.searchData0.forEach((item) => {
      if (item.name === 'salesGoodsReturnStatusList') {
  ...
  methods: {
    ...
    initQueryParams(isResetDate = true) {
      ...
      if (isResetDate) {
        ...
        this.$set(this.queryParams, 'startDateRequiredDate', startDateRequiredDate)
        this.$set(this.queryParams, 'endDateRequiredDate', endDateRequiredDate)
        this.$set(this.queryParams, 'dateRange2', dateRange2)
        this.$set(this.queryParams, 'startDateDueDate', startDateDueDate)
        this.$set(this.queryParams, 'endDateDueDate', endDateDueDate)
      }

      this.queryParams.selectId = 'conditionForSalesGoodsReturnNo'
      this.queryParams.selectId2 = 'createdIdList'
      this.searchData.forEach((item) => {
        if (item.name === 'selectAnInputName') {
          item.selectId = 'conditionForSalesGoodsReturnNo'
          item.inputVal = ''
        }
    ...
    tableRowClassName({ row, rowIndex }) {
      ...
      for (const item of this.ids.values()) {
        if (item === row.salesGoodsReturnId) {
    ...
    getList(query) {
      ...
      querySalesGoodsReturnList(params).then((response) => {
        ...
        rows.forEach((item) => {
          ...
          item.salesGoodsReturnStatusStr = this.selectDictLabel(
            this.statusDicts,
            item.salesGoodsReturnStatus
          )
    ...
    querySearchList() {
      querySearchList({ labelType: this.activeName }).then((res) => {
        ...
        this.conditionForSalesGoodsReturnNoList = (
          data.conditionForSalesGoodsReturnNoList || []
        ).map((item) => {
          return {
            value: item,
            label: item
        ...
        this.conditionForSalesOrderNoList = (data.conditionForSalesOrderNoList || []).map(
          (item) => {
            return {
              value: item,
              label: item
    ...
    setFuzzyData() {
      this.searchData.forEach((item) => {
        if (item.name === 'selectAnInputName') {
          if (item.selectId === 'conditionForSalesGoodsReturnNo') {
            this.$set(item, 'fuzzyData', this.conditionForSalesGoodsReturnNoList)
          } else if (item.selectId === 'conditionForSalesOrderNo') {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.salesGoodsReturnId)
    ...
    handleAdd() {
      this.$router.push({
        path: '/salesManagement/addSalesGoodsReturn',
        query: {
          timeId: Date.now()
        }
      })
    ...
    handleUpdate(row, column) {
      ...
      this.$router.push({
        path: '/salesManagement/editSalesGoodsReturn',
        query: {
          id: row.salesGoodsReturnId,
          timeId: Date.now()
        }
      })
    },
    ...
    handleOperationLog(row) {
      this.$refs.systemOperationLogTableDlg.handleOpen(querySalesInquiryOperationLog, {
        salesGoodsReturnId: row.salesGoodsReturnId
      })
    },
    /** 导出按钮操作 */
    /** 导出按钮操作 */
    ...
    handleExport(data) {
      ...
      if (exportType === 2) {
        params.exportIdList = vm.tableList.map((item) => item.salesGoodsReturnId)
      }
      if (exportType === 3) {
      ...
      this.$importOrExportDownFile(
        '/sales/salesGoodsReturn/export',
        params,
        `${this.getFileNameDate('Sales Return Order')}.xlsx`
      )
        .then((res) => {

...
import {
  querySGRCanSelectDeliveryOrderProductList,
...
export default {
  ...
  methods: {
    ...
    queryAllProductCategoryTreeList() {
      if (this.treeList.length > 0 || this.tableList.length <= 0) return
      const list = this.tableList.flatMap((x) => x.categoryIdList || [])
      const idList = [...new Set(list)]
      console.log(idList, '=============420')
      queryBusinessSelectProductCategoryTreeList({ productCategoryIdList: idList }).then((res) => {
        const list = res.data || []
        this.treeList = list
    ...
    getList() {
      ...
      param.deliveryOrderId = this.formData.deliveryOrderId
      param.salesGoodsReturnId = this.formData.salesGoodsReturnId
      param.warehouseId = this.formData.warehouseId
      param.selectedIdList = this.$$getSelectedIdList(this.selectList, this.rowIdKey)
      this.loading = true
      querySGRCanSelectDeliveryOrderProductList(param)
        .then((response) => {
          this.loading = false

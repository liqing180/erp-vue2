    ...
    <el-row :gutter="10">
      <el-col :span="1.5">
        <el-button
          ...
          icon="el-icon-plus"
          size="mini"
          v-hasPermi="['salesManagement:salesCost:add']"
          @click="handleAdd"
...
import { querySalesCostSettingsList, querySearchList } from '@/api/salesManagement/salesCost'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      importOptions: {
        // 下载模板
        downFileUrl: '/system/productExtended/importSalesCostTemplate',
        // 上传校验
        uploadUrl: process.env.VUE_APP_BASE_API + '/sales/salesCostSettings/checkImportData',
        // 提交导入
        importUrl: '/sales/salesCostSettings/importData',
        // 下载文件名
        fileName: 'Cost Settings'
      },
      typeDicts: [
        {
          label: vm.$t('SALES.expenseProportion6'),
  ...
  methods: {
    ...
    getList() {
      ...
      this.loading = true
      querySalesCostSettingsList(params)
        .then((response) => {
    ...
    handleAdd(row) {
      this.$router.push({
        path: '/salesManagement/addCostSettings',
    ...
    handleDblclick(row, column) {
      ...
      this.$router.push({
        path: '/salesManagement/editCostSettings',
        query: { id: row.salesCostSettingsId, timeId: +new Date() }
      })
    },
    handleDetail(row) {
    ...
    tableRowClassName({ row, rowIndex }) {
      ...
      for (const item of this.ids.values()) {
        if (item === row.salesCostSettingsId) {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.salesCostSettingsId)
    ...
    handleExport(data) {
      ...
      if (exportType === 2) {
        params.exportIdList = vm.tableList.map((item) => item.salesCostSettingsId)
      ...
      this.$importOrExportDownFile(
        '/sales/salesCostSettings/export',
        params,
        `${this.getFileNameDate('Sales Cost Settings')}.xlsx`
      )
        .then((res) => {

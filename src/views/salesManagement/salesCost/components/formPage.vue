          ...
          <el-collapse-item name="1">
            ...
            >
              <el-row>
                ...
                <el-col :span="8">
                  <el-form-item
                    :label="$t('SALES.expenseProportionAmount')"
                  ...
                  >
                    <el-input-number
                      ...
                      :key="'expenseProportionAmount_' + timeStamp"
              ...
              <el-row>
                <el-col :span="24">
                  <el-form-item :label="$t('PURCHASE.inventoryCategory')">
                    <ToolTipShowCategory :list="productCategoryNameList || []">
...
<script>
import {
  saveSalesCostSettings,
  updateSalesCostSettings,
  querySalesCostSettingsById
...
export default {
  ...
  computed: {
    ...
    editAuth() {
      return this.checkPermi(['salesManagement:salesCost:edit'])
    },
    comDisFrom() {
      if (this.rowId) {
  ...
  methods: {
    ...
    handleUpdate() {
      this.reset()
      querySalesCostSettingsById({ salesCostSettingsId: this.rowId }).then((res) => {
        const data = res.data
        this.form = data
        this.timeStamp = +new Date()
        const { productList, categoryNameList } = this.form
        this.initProductList = productList || []
        this.productCategoryNameList = categoryNameList || []
        setTimeout(() => {
    ...
    updateProductTable(list) {
      ...
      if (this.initProductList && this.initProductList.length > 0) {
        this.handlerProductCategoryNames()
      } else {
        this.$set(this.form, 'categoryNameShowStr', undefined)
        this.$set(this.form, 'productCategoryNames', undefined)
        this.$set(this.form, 'productCategoryIds', undefined)
      }
    },
    handlerProductCategoryNames() {
      let ids = []
      let names = []
      this.initProductList.forEach((x) => {
        const { categoryIdList, categoryNameList } = x
        if (ids && categoryNameList) {
          const showList = categoryNameList.map((item) => {
            return JSON.stringify(item)
          })
          ids.push(...categoryIdList)
          names.push(...showList)
        }
      })
      ids = [...new Set(ids)]
      names = [...new Set(names)]
      this.productCategoryNameList = names.map((item) => {
        return JSON.parse(item)
      })
      this.$set(
        this.form,
        'categoryNameShowStr',
        this.productCategoryNameList.map((item) => item.join(' > ')).join(' | ') || ''
      )
      this.form.productCategoryNames = this.form.categoryNameShowStr
      this.$set(this.form, 'productCategoryIds', ids.join(',') || '')
    },
    reset() {
      this.form = {
        isActive: '1'
    ...
    cancel() {
      ...
      this.$router.push({ path: '/salesManagement/salesCost' })
    },
    async submitForm() {
      ...
      this.$modal
        ...
        .then(() => {
          ...
          if (this.rowId) {
            return updateSalesCostSettings(params)
          } else {
            return saveSalesCostSettings(params)
          }
        })
        .then((response) => {

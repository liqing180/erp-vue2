<template>
  ...
  >
    <div class="right-title">
      ...
      <span class="mr20" v-show="form.salesCost" :title="form.salesCost">
        {{ $t('SALES.overheadsAmount') }}：{{ $numberStr(form.salesCost, 2) }}
      </span>
    </div>
...
import { salesCostQuerySalesCostMsg } from '@/api/salesManagement/salesCost'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      salesCostQuerySalesCostMsg(param)
        .then((response) => {
          this.form = response.data
          const { salesCostSettingsList } = response.data
          const rows = salesCostSettingsList || []
          rows.forEach((item) => {

<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <template v-if="['salesQuotationNo', 'salesOrderNo'].includes(item.prop)">
...
export default {
  ...
  computed: {
    addAuth() {
      return this.checkPermi(['salesManagement:productTypesChoosing:add'])
    },

    exportAuth() {
      return this.checkPermi(['salesManagement:productTypesChoosing:export'])
    },
    curColumns() {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push({
        path: '/salesManagement/addProductTypesChoosing',
        query: {
          timeId: Date.now()
        }
      })
    ...
    handleUpdate(row, column) {
      ...
      this.$router.push({
        path: '/salesManagement/editProductTypesChoosing',
        query: {
    ...
    handleExport(data) {
      ...
      this.$importOrExportDownFile(
        ...
        params,
        `${this.getFileNameDate('Sales Goods Return')}.xlsx`
      )
        .then((res) => {

<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <!--  ((scope.row.buttonAuthMsg || {}).isCanCreatePreDeliveryNotice === '1' &&
                assignBtnShow) || -->
          <span
            :title="$t('SALES.schedule')"
            v-if="(scope.row.buttonAuthMsg || {}).isCanOpenBlanketOrder === '1'"
          >
            <svg-icon
              icon-class="stockIn"
...
export default {
  ...
  computed: {
    addAuth() {
      return this.checkPermi(['salesManagement:salesOrder:add'])
    },

    exportAuth() {
      return this.checkPermi(['salesManagement:salesOrder:export'])
    },
    curColumns() {
  ...
  methods: {
    ...
    initQueryParams(isResetDate = true) {
      ...
      if (isResetDate) {
        ...
        this.$set(this.queryParams, 'startDateDelivery', startDateDelivery)
        this.$set(this.queryParams, 'endDateDelivery', endDateDelivery)
      }
      ...
      this.searchData.forEach((item) => {
        ...
        if (item.name === 'salesQuotationTypeList') {
          item.selectData = this.typeDicts.filter((x) => x.value !== 'Service')
        }
      })
    },
    tabClickHandler() {
    ...
    getList(query) {
      ...
      querySalesOrderList(params).then((response) => {
        ...
        rows.forEach((item) => {
          ...
          item.salesQuotationTypeStr = this.selectDictLabel(this.typeDicts, item.salesQuotationType)
        })
        this.tableList = rows
        this.loading = false

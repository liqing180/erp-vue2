    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          <template v-else-if="item.prop === 'qty'">{{ $numberStr(scope.row.qty, 3) }}</template>
...
export default {
  ...
  watch: {
    ...
    'formData.salesOrderProductList': {
      ...
      handler: function () {
        let timeId = Date.now()
        this.formData.salesOrderProductList.forEach((item) => {
          timeId++
...
.hide-check-all {
  /deep/ th.el-table-column--selection .cell .el-checkbox__inner {
    display: none;
    position: relative;
  }
}

  ...
  >
    ...
    <div class="mt10">
      <el-table border ref="tables" :data="tableList" :max-height="tableMaxHeight">
        ...
        >
          <template slot-scope="scope">
            <template v-if="['pendPurchaseQty'].includes(item.prop) && purchaseRequisitionAuth">
...
export default {
  ...
  methods: {
    ...
    handleOpen(row) {
      ...
      this.visible = true
      this.changeTableList(querySalesOrderProductOtherMsg)
    ...
    changeTableList() {
      this.$$getColumnContentMaxWidth(this.columns, [])
    }
  }
}
</script>

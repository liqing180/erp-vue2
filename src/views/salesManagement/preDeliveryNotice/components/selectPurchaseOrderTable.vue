<template>
  <el-dialog
    v-dialogDrag
    v-dialogDragWidth
    :title="$t('PURCHASE.purchaseOrder')"
...
import { queryPurchaseOrderListFromPreDeliveryNotice } from '@/api/salesManagement/preDeliveryNotice'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      param.documentId = this.formData.documentId
      param.documentType = this.formData.documentType
      this.$trimOfObj(param)
      param.menuPerms = this.menuKey.preDeliveryNotice
      queryPurchaseOrderListFromPreDeliveryNotice(param)
        .then((response) => {

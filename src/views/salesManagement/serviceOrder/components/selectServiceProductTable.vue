...
import { queryPhantomCategoryTreeList } from '@/api/productManagement/productCategory'
import { queryCanSelectProductListFromServiceOrder } from '@/api/salesManagement/serviceOrder'
...
export default {
  ...
  methods: {
    ...
    queryAllProductCategoryTreeList() {
      queryPhantomCategoryTreeList().then((res) => {
        const list = res.data || []
        this.treeList = list
    ...
    getList() {
      ...
      // param.serviceType = 'Internal'
      param.purpose = '2'
      param.selectedIdList = this.$$getSelectedIdList(this.selectList, this.rowIdKey)
      this.loading = true
      queryCanSelectProductListFromServiceOrder(param)
        .then((response) => {
          this.loading = false

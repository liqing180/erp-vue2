  ...
  <div class="pb20">
    <el-row :gutter="10" class="mb8">
      ...
      <div class="footer-content">
        <div>{{ $t('SALES.rejectedQty') }} : {{ $numberStr(comTotalQty, 3) }}</div>
        <div class="ml20">
    ...
    >
      ...
      >
        <template slot="header" slot-scope="{ column }">
          <span
            v-if="['rejectedQty', 'returnedReason', 'scheduledReceivedDate'].includes(item.prop)"
          >
        ...
        <template slot-scope="scope">
          ...
          <template v-else-if="item.prop === 'returnedRemarks'">
            <el-input
              v-if="!comDisFrom"
              style="width: 98%"
              :title="scope.row.returnedRemarks"
              v-model="scope.row.returnedRemarks"
              :maxlength="200"
            />
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        /* {
          ...
          label: vm.$t('SALES.scheduledReceivedDate'),
          visible: true,
          minWidth: 200,
          tooltip: true
        } */
  ...
  methods: {
    ...
    getLineTotal(row) {
      this.$nextTick(() => {
        if (this.$resultOfBoolean(row.rejectedQty) && this.$resultOfBoolean(row.preUnitPrice)) {
          const rejectedAmount = row.preUnitPrice * row.rejectedQty
          this.$set(row, 'rejectedAmount', rejectedAmount)
        } else {
          this.$set(row, 'rejectedAmount', undefined)
        }
      })
    },

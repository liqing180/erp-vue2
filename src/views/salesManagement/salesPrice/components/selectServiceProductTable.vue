...
import { queryCanSelectServiceList, dealAddProductMsg } from '@/api/salesManagement/salesInquiry'
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      dealAddProductMsg({
        salesQuotationServiceList: this.selectList,
        ...
        .then((res) => {
          this.btnLoading = false
          const data = res.data || {}
          this.$emit('onSuccess', data.salesQuotationServiceList || [])
          this.visible = false
        })
        .catch(() => {
          this.btnLoading = false

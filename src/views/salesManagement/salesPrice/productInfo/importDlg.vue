<template>
  ...
  >
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="submitForm" :disabled="isUploading" :loading="btnLoading"
        >{{ $t('uiBtn.submit') }}
      </el-button>
    </div>
  </el-dialog>
...
<script>
// import { importData } from '@/api/productManagement/productCategory'
import { importData } from '@/api/basic/basic'
...
export default {
  ...
  methods: {
    ...
    downLoad() {
      ...
      const params = {
        alreadySalesPartIdList: this.alreadyProductIdList
      }
      const { salesOrganizationIdList } = this.formData
      if (salesOrganizationIdList && salesOrganizationIdList.length > 0) {
        params.salesOrganizationId = salesOrganizationIdList.join(',')
      }
            ...
            .then((res) => {
              vm.$modal
                ...
                .then(() => {
                  ...
                  formData.append('targetCurrencyId', vm.currencyId)
                  return importData(vm.importOptions.importUrl, formData)
                })
                .then((response) => {
                  vm.btnLoading = false
                  vm.$modal.msgSuccess(vm.$t('ui.ImportSuccessful'))
                  vm.cancel()
                  vm.$emit('onSuccess', response)
                })
                .catch((err) => {
                  vm.btnLoading = false

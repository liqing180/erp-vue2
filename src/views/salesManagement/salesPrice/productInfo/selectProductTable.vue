...
import {
  querySalesPriceCanSelectProductList,
  dealAddProductMsg,
  querySalesPriceCanSelectProductListNoPage
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.loading = true
      querySalesPriceCanSelectProductList(param)
        .then((response) => {
          this.loading = false
    ...
    submit() {
      ...
      dealAddProductMsg({ salesPriceProductList: this.selectList })
        .then((res) => {
          const { salesPriceProductList } = res.data
          this.$emit('onSuccess', salesPriceProductList)
          this.visible = false
          setTimeout(() => {
    ...
    addAll() {
      ...
      querySalesPriceCanSelectProductListNoPage(param)
        .then((res) => {
          const data = res.data || []
          dealAddProductMsg({ salesPriceProductList: data })
            .then((res2) => {
              const { salesPriceProductList } = res2.data
              this.$emit('onSuccess', salesPriceProductList)
              this.visible = false
              setTimeout(() => {
                this.btnLoading = false

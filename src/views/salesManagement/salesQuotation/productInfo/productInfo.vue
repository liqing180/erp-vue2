...
export default {
          ...
          list = [
            ...
            },
            {
              ...
              disabled:
                ...
                this.formData.salesQuotationProductList.length <= 0
            }
          ]
        } else {
          list = [
  ...
  methods: {
    ...
    handleCalculate(type) {
      ...
      // }
      if ((!type || type !== 'previewPDF') && this.formData.blanketOrder !== '1') {
        const qtyReq1 = this.totalForm.productOrServiceList.find((item) => {
          return !this.$resultOfBoolean(item.qty)
        })
        if (qtyReq1) {

<template>
  <div class="pb20">
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5" v-if="addBtnShow">
        <el-button
          @click="handleAddBtn"
      ...
      <div class="footer-content">
        <el-form
          label-width="180px"
          class="mr20"
          v-if="['Trading', 'Service'].indexOf(totalForm.salesQuotationType) !== -1"
        >
    ...
    >
          ...
          <span
            v-if="
              [
                'qty',
                'unitPrice',
                formData.documentType !== 'ServiceOrder' ? 'deliveryDateForDay' : ''
              ].includes(item.prop)
            "
          >
        ...
        <template slot-scope="scope">
          ...
          <template v-else-if="item.prop === 'deliveryDateForDay' && !scope.row.isTotal">
            <el-input-number
              ...
              :min="1"
              :max="999"
              :disabled="formData.documentType === 'ServiceOrder'"
      ...
      <el-table-column
        v-if="!comDisFrom && addCustomBtnShow"
        :label="$t('ui.action')"
        key="action"
        align="center"
        width="120"
...
export default {
  ...
  computed: {
    ...
    disType() {
      let dis = false
      if (
        this.formData.salesQuotationStatus === '4' &&
  ...
  methods: {
    ...
    handleAddBtn() {
      let alreadySelectIdList = this.tableList.map((item) => item[this.rowIdKey])
      alreadySelectIdList = alreadySelectIdList.filter((item) => item)

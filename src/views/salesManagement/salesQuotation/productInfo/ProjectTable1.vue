  ...
  watch: {
    ...
    'formData.salesQuotationProjectList': {
      ...
      handler: function (selected) {
        this.tableList = this.formData.salesQuotationProjectList
  ...
  methods: {
    ...
    projectChange(id, index) {
      if (id) {
        ...
        this.totalForm.salesQuotationProductList = projectTemplateContentList || []
        // this.totalForm = Object.assign(this.totalForm, {
        //   otherCosts,
        //   subTotal,
        //   expenseAmount,
        //   totalFreight,
    ...
    queryProjectTemplateById(projectTemplateId, index) {
      queryProjectTemplateById({ projectTemplateId }).then((res) => {
        const { projectMsg, projectQuotationContent, projectTermsConditions } = res.data
        this.$set(this.tableList[index], 'projectMsg', projectMsg)
        this.$set(this.tableList[index], 'projectQuotationContent', projectQuotationContent)
        this.$set(this.tableList[index], 'projectTermsConditions', projectTermsConditions)
        setTimeout(() => {
          this.$emit('handleCalculate')
        }, 100)
      })

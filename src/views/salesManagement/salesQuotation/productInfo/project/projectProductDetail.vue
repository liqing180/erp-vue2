<template>
  <div class="pb20">
    ...
    <UTable
      :key="tableTimerKey + modifyHighlight + timestamp"
      border
      ref="tables"
        ...
        <template slot-scope="scope">
          ...
          <template v-else-if="item.prop === 'salesPartNo' && !scope.row.isTotal">
            <el-input
              ...
              :maxlength="200"
              @input="commonPropChange(scope.row)"
            />
          ...
          <template v-else-if="item.prop === 'productName'">
            ...
            >
              <el-input
                ...
                :maxlength="200"
                @input="productNameChange(scope.row)"
              />
          ...
          <template v-else-if="item.prop === 'description'">
            <descriptionEditDlg
              ...
              :maxlength="7000"
              @input="descriptionChange($event, scope.row)"
            />
            <template v-else>
          ...
          <template v-else-if="item.prop === 'uom' && !scope.row.isTotal">
            <template v-if="scope.row.isAdhocEntry === '1' && !comDisFrom && scope.row.isEdit">
              <el-select
                ...
                placeholder=""
                filterable
                @change="rowUomChange(scope.row, 'all')"
              >
          ...
          <template v-else-if="item.prop === 'remarks' && !scope.row.isTotal">
            <descriptionEditDlg
              ...
              :maxlength="200"
              :disabled="!(!comDisFrom && scope.row.isEdit)"
              @input="commonPropChange(scope.row)"
            />
          </template>
          <template v-else>{{ scope.row[item.prop] }}</template>
...
import selectProductTable from '@/views/salesManagement/salesInquiry/productInfo/project/selectProductTable.vue'
...
export default {
  ...
  methods: {
    descriptionChange(e, row) {
      this.commonPropChange(row)
    },
    commonPropChange(row) {
      console.log(row, '=============844')
      this.updateTreeNode(this.tableList, row)
      console.log(this.tableList, '==========1070')
    },
    /**
     * 更新树形表格中的某一行数据
     * @param {Array} treeData - 树形数组
     * @param {Object} newRow - 变化后的新行数据对象
     * @param {String|Number} idKey - 唯一标识的字段名，默认为 'id'
     */
    updateTreeNode(treeData, newRow) {
      for (let i = 0; i < treeData.length; i++) {
        const node = treeData[i]

        // 1. 匹配当前节点
        if (node[this.rowIdKey] === newRow[this.rowIdKey]) {
          // 2. 使用 Vue 2 的 $set 进行响应式替换
          // 注意：不要直接 treeData[i] = newRow，这不会触发视图更新
          this.$set(treeData, i, newRow)
          return true // 找到并替换后，直接返回，提高性能
        }

        // 3. 如果当前节点有子节点，递归查找
        if (node.childProductList && node.childProductList.length > 0) {
          const found = this.updateTreeNode(node.childProductList, newRow)
          if (found) return true // 如果在子树中找到了，也直接返回
        }
      }
      return false // 未找到
    },
    productNameChange(row) {
    ...
    handleRowClick(row, column) {
      ...
      // if (this.curEditRow) {
      //   this.$set(this.curEditRow, 'isEdit', true)
      // }
        ...
        .then((res) => {
          if (row.queryTime === timer) {
            ...
            const handlerProductList = (list = []) => {
              ...
              buildMap(salesQuotationProductList || [])
              updateList(list)
            }
            handlerProductList(this.tableList)
            const data = salesQuotationProductList.find((x) => x[rowKey] === row[rowKey])
        ...
        if (topRow && topRow.deliveryDateForDay) {
          ...
          if (oldExpectedDeliveryDate) {
            ...
            this.$modal
              ...
              .then(() => {
                const newTableList = this.tableList.map((row) => {
                  return {
                    ...row, // 保留原有数据
                    deliveryDateForDay: topRow.deliveryDateForDay,
                    deliveryDateForDayError: false,
                    ROW_ERROR: false
                  }
                })
                this.tableList.length = 0
                this.tableList.push(...newTableList)

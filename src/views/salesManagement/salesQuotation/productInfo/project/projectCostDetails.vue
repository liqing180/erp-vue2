<template>
  <div class="pb20">
    ...
    <UTable
      ...
      :key="'table' + modifyHighlight + timestamp"
    ...
    >
      ...
      <UTableColumn
        ...
        header-align="center"
        :tree-node="item.prop === 'productName'"
      >
...
export default {
  ...
  methods: {
    ...
    productNameChange(row) {
      this.$set(row, 'productNameErrorChildren', false)
    ...
    handleAddChild(e, row) {
      ...
      if (e === 'addAdhocEntry') {
        ...
        if (row.childProductList && typeof row.childProductList === 'object') {
          ...
          this.$set(row, 'childProductList', [...row.childProductList, params])
        } else {
          ...
          params.rowTimeId = Date.now()
          params = JSON.parse(JSON.stringify(params))
          this.$set(row, 'childProductList', [params])
        }
      ...
      this.$nextTick(() => {
        this.$refs.tables && this.$refs.tables.setTreeExpansion(row, true)
      })
      this.changeTableList()
    },
    // 当前行选择已有产品，可选单个和多个
    ...
    updateTableList(e) {
      ...
      const rows = JSON.parse(JSON.stringify(salesQuotationProductList))

...
import { queryBusinessSelectServiceCategoryTreeList } from '@/api/productManagement/productCategory'
import { queryCanSelectServiceList, dealAddProductMsg2 } from '@/api/salesManagement/salesInquiry'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      param.alreadyProductIdList = this.alreadySelectIdList.filter((item) => item)
      param.selectedIdList = this.$$getSelectedIdList(this.selectList, this.rowIdKey)

      this.loading = true
      param.purpose = '2'
      queryCanSelectServiceList(param)
        .then((response) => {
          this.loading = false
          ...
          rows.forEach((item) => {
            ...
            item.serviceTypeShowStr = this.selectDictLabel(

...
import { queryCanSelectWarehouseListNoDataPermissions } from '@/api/salesManagement/salesInquiry'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      if (this.salesQuotationType === 'MixDesign') {
        param.warehouseTypeList = ['1']
      }
      param.menuPerms = this.menuKey.salesQuotation
      queryCanSelectWarehouseListNoDataPermissions(param)
        .then((response) => {

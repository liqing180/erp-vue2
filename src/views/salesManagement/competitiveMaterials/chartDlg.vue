...
import { queryProductHistoryPriceImage } from '@/api/salesManagement/competitiveMaterials'
...
export default {
  ...
  methods: {
    openChart(row) {
      this.$set(this.form, 'competitorId', row.competitorId)
      this.visible = true
      this.queryProductHistoryPriceImage()
    },
    queryProductHistoryPriceImage() {
      const { dateRange, competitorId } = this.form
      const params = {
        competitorId
      }

<template>
  ...
  >
    ...
    <div class="mt20" style="display: flex; align-items: flex-start" v-loading="loading">
      ...
      <div class="w100" style="overflow: hidden">
        <el-table
          ...
          :class="isChildren ? 'hide-check-all' : ''"
        >
        ...
        <pagination
          :selectedNum="
            $$getFilteredSelectedNum(filteredSelectedList, tableList, selectList1, rowIdKey)
          "
          :saveKey="saveKey"
          :savePath="savePath"
          v-show="total > 0"
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" :disabled="selectList1.length <= 0" @click="submit"
        >{{ $t('uiBtn.save') }}
      </el-button>
    </div>
  </el-dialog>
...
export default {
  ...
  methods: {
    ...
    // isChildren true产品需要单选
    handleAdd(data = [], isChildren = false) {
      this.isChildren = isChildren
      this.alreadyProductIdList = data
    ...
    getList() {
      ...
      if (this.isFather) {
        param.purpose = '2'
        param.assembledProduct = '1'
      } else {
        param.purpose = undefined
        param.assembledProduct = undefined
      }
      this.$trimOfObj(param)

      this.loading = true
      param.alreadyProductIdList = this.alreadyProductIdList
      param.selectedIdList = this.$$getSelectedIdList(this.selectList1, this.rowIdKey)

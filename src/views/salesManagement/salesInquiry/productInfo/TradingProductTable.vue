      ...
      >
        ...
        <template slot-scope="scope">
          <template v-if="item.prop === 'salesInquiryProductType' && !scope.row.isTotal">
            <template v-if="!comDisFrom && !disType">
              <el-select
                style="width: 98%"
          ...
          <template v-else-if="item.prop === 'salesPartNo' && !scope.row.isTotal">
            <template v-if="!comDisFrom && !disSalesPartNo">
              <CommonSelect
                style="width: 98%"
          ...
          <template v-else-if="item.prop === 'purchaseQuotationNo' && !scope.row.isTotal">
            <!-- <el-select
              ...
              placeholder=""
              v-if="!comDisFrom && disInQuotation"
              size="mini"
          ...
          <template v-else-if="item.prop === 'priceType' && !scope.row.isTotal">
            <CommonSelect
              v-if="!comDisFrom && !disPriceType && scope.row.salesPartNo"
              style="width: 98%"
          ...
          <template v-else-if="item.prop === 'discountPer' && !scope.row.isTotal">
            <el-input-number
              ...
              v-if="!comDisFrom && scope.row.salesInquiryProductType === '1'"
              style="width: 98%"
...
export default {
  ...
  computed: {
    ...
    addAdhocEntryAuth() {
      ...
      if (this.formData.salesInquiryType === 'Consignment') {
        return false
      }
      if (this.disInQuotation) {
        return false
  ...
  methods: {
      ...
      this.columns.forEach((item) => {
        ...
        switch (prop) {
          ...
          case 'purchaseQuotationNo':
            ...
            break
          case 'salesPartNo':
            item.colMinWidth = !vm.comDisFrom && !vm.disPriceType ? 200 : undefined
            break
          case 'description':
          ...
          case 'priceType':
            ...
            break
          case 'qty':
            item.colMinWidth = vm.comDisFrom || vm.disQty ? undefined : 200
            break
          case 'partNo':
    ...
    salesInquiryProductTypeChange(row) {
      this.$set(row, 'salesInquiryProductTypeError', false)
    ...
    salesPartNoChange(selectRow, row) {
      this.$set(row, 'salesPartNoError', false)
      this.$set(row, 'ROW-ERROR', false)
        ...
        calculate(params)
          .then((res) => {
            if (row.queryTime === timer) {
              ...
              const data = salesInquiryProductList.find((x) => x.productId === row.productId)
    ...
    setMaxExpectedDeliveryDate() {
      ...
      const expectedDeliveryDateList2 = this.formData.salesInquiryServiceList.map(

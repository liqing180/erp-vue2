  ...
  >
    <!-- <el-row>
      <el-tabs v-model="activeName" type="card" @tab-click="handleTabClick">
        <el-tab-pane :label="$t('PRODUCT.product')" name="1"></el-tab-pane>
...
export default {
  ...
  computed: {
    ...
    currentSelectedRows() {
      if (this.activeName === '1') {
        return this.selectList1.filter((item) => !!item.salesPartId)
      }
      return this.selectList1.filter((item) => !item.salesPartId && !!item.productId)
    }
  },
  watch: {},
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      if (this.activeName === '1') {
        ...
        queryFn = querySalesInquiryCanSelectProductList
      } else {
        param.targetCurrencyId = this.formData.currencyId
        queryFn = queryCanSelectServiceListFromSalesInquiry
      }
      param.alreadyProductIdList = []
      param.selectedIdList = this.$$getSelectedIdList(this.currentSelectedRows, this.rowIdKey)
    ...
    submit() {
      ...
      dealAddProductMsg2({
        productOrServiceList: this.selectList1,
        ...
        salesInquiryType: this.formData.salesInquiryType || this.formData.salesQuotationType
      })
        .then((res) => {
          setTimeout(() => {
          ...
          const data = res.data || {}
          this.$emit('onSuccess', data.productOrServiceList || [])
          this.visible = false
        })
        .catch(() => {
          this.btnLoading = false

        ...
        >
          ...
          >
            <template slot-scope="scope">
              ...
              <template v-else-if="item.prop === 'unitPriceForBase'">
...
import {
  querySalesInquiryCanSelectProductList,
  dealAddProductMsg2
...
export default {
  ...
  data() {
    ...
    return {
      ...
      searchData: [
        {
          name: 'condition',
          placeholder: `${this.$t('ui.salesPartNoProductSearch')}`,
          type: 'InputEle'
        }
      ],
      treeList: [],
  ...
  methods: {
    ...
    getList() {
      ...
      // param.alreadySalesPartIdList = this.alreadySelectIdList
      param.businessPartnerId = this.formData.businessPartnerId
      param.warehouseId = this.formData.warehouseId
      param.salesInquiryType = this.formData.salesInquiryType
      param.salesInquiryId = this.formData.salesInquiryId
      param.costProjectId = this.formData.costProjectId
      param.currencyId = this.formData.currencyId
      param.priceTypeId = this.formData.priceTypeId
      param.customerGroup = this.formData.customerGroup
      param.selectedIdList = this.$$getSelectedIdList(this.selectList, this.rowIdKey)

      this.loading = true
      querySalesInquiryCanSelectProductList(param)
        .then((response) => {
          this.loading = false
    ...
    submit() {
      ...
      const isAvailableItem = this.selectList.filter((item) => item.availableQty > 0)
      if (isAvailableItem.length > 0 && this.formData.salesInquiryType === 'Trading') {
        let names = isAvailableItem.map((item) => item.productName)
        names = Array.from(new Set(names))
        const namesStr = names.join(', ')
        const confirmMsg = this.$t('SALES.SIProductSaveConfirm').replace('$1', namesStr)
        this.$modal
          .confirm(confirmMsg)
          .then(() => {
            this.dealAddProductMsg()
          })
          .catch(() => {
            this.btnLoading = false
          })
    ...
    dealAddProductMsg() {
      ...
      dealAddProductMsg2({
        ...
        salesInquiryType: this.formData.salesInquiryType
      })
        .then((res) => {
          const data = res.data || {}
          this.$emit('onSuccess', data.salesInquiryProductList || [])
          this.visible = false
          setTimeout(() => {
            this.btnLoading = false
          }, 500)

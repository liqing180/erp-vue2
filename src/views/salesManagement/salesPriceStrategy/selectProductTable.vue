...
import { querySalesCanSelectProductList } from '@/api/salesManagement/salesInquiry'
import { queryProductSalesPriceList } from '@/api/salesManagement/salesPriceStrategy'
export default {
  directives: {
    // transferDom
  },
  ...
  data() {
    ...
    return {
      ...
      columns: [
        ...
        {
          prop: 'inventoryUom',
          label: vm.$t('PRODUCT.inventoryUOM2'),
          visible: true,
          minWidth: 200,
          sortable: 'custom',
          tooltip: true
  ...
  methods: {
    ...
    getList() {
      ...
      this.loading = true
      querySalesCanSelectProductList(param)
        .then((response) => {
          this.loading = false
    ...
    submit() {
      ...
      queryProductSalesPriceList(this.selectList).then((res) => {
        const selectList = res.data || []
        this.$emit('onSuccess', selectList)
        this.visible = false
      })
    }
  }

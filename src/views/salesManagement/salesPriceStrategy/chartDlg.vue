<template>
  ...
  >
    <el-form ref="form" :model="form" @submit.native.prevent label-width="120px">
      <el-form-item :label="$t('SALES.timeLimit')">
        <el-date-picker
...
import { queryProductHistoryPriceImage } from '@/api/salesManagement/salesPriceStrategy'
...
export default {
  ...
  methods: {
    openChart(row) {
      ...
      this.$set(this.form, 'salesPartId', row.salesPartId)
      this.$set(this.form, 'priceType', row.priceType)
      this.visible = true
      this.queryProductHistoryPriceImage()
    },
    queryProductHistoryPriceImage() {
      const { dateRange, productId, salesPartId, priceType } = this.form
      const params = {
        productId,
        salesPartId,
        priceType
      ...
      queryProductHistoryPriceImage(params).then((response) => {
        this.$nextTick(() => {

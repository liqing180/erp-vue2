<template>
  <FormPageLayout ref="FormPageLayoutRef" v-loading="submitLoading">
                  ...
                  <el-col :span="8">
                    <el-form-item :label="`${$t('PURCHASE.requiredBy')}`" prop="requiredBy">
                      ...
                      <!-- <SelectInput
                        ...
                        :disabled="comDisFrom || !form.departmentId"
                      /> -->
                      <CommonSelectAndList
                        v-if="!isModified('requiredBy')"
                        :id="form.requiredId"
                        ...
                        :disabled="comDisFrom || requiredByLoading || !form.departmentId"
                  ...
                  <el-col :span="16">
                    ...
                    >
                      <ComparisonInput
                        ...
                        :afterValue="getAfterValue('receiveAddressName', form.costProjectCode)"
                      />
                  ...
                  <el-col :span="8">
                    ...
                    >
                      <ComparisonInput
                        ...
                        :afterValue="getAfterValue('mobilePhone', form.mobilePhone)"
                      />
                      <MobilePhoneInput
                        v-else
            ...
            <el-collapse-item name="2">
              ...
              <div class="pb20" v-loading="btnLoading">
                <el-row :gutter="10" class="mb8">
                  ...
                  <el-col :span="1.5" v-if="!comDisFrom && addAdhocEntryAuth">
                    <el-button
                      :disabled="form.dropShipping !== '1' && !form.warehouseId"
                  ...
                  >
                    ...
                    <template slot-scope="scope">
                      ...
                      <template v-else-if="item.prop === 'externalPartNo11'">
                        ...
                        <span>{{ (scope.row.externalPartNoList || []).join(', ') }}</span>
                      </template>
    ...
    <RevisionComparisonDlg module-key="purchaseRequisite" ref="RevisionComparisonDlg" />
  ...
  computed: {
    ...
    configCurColumn() {
      ...
      if (this.form.approvedStatus !== '3') {
        arr = arr.filter((x) => ['closedQTY', 'rejectedQtyForBasicShowStr'].indexOf(x.prop) === -1)
      }
    ...
    submitBtnShow() {
      ...
      if (this.form.purchaseRequisiteId && this.buttonAuthMsg.isCanUpdate === '1') {
        show = true
      }
      return show
    },
    ...
    withdrawApproveBtnShow() {
      if (!this.checkPermi(['purchaseManagement:purchaseRequisition:withdrawApprove'])) {
        return false
      }
      let show = false

      if (this.form.purchaseRequisiteId && this.buttonAuthMsg.isCanWithdrawApproved === '1') {
        show = true
      }
      return show
    },
  ...
  methods: {
      ...
      queryPurchaseRequisiteById({
        ...
        list.forEach((item, index) => {
          ...
          if (item.externalPartNoListJson) {
            this.$set(item, 'externalPartNoList', JSON.parse(item.externalPartNoListJson))
          } else {
            this.$set(item, 'externalPartNoList', [])
          }
        })
    ...
    dropShippingChange() {
      ...
      if (this.form.dropShipping !== '1') {
        ...
        if (
          ...
          this.$set(this.form, 'contactPersonName', this.form.consignee)
        }
      }
    },
    /* 表单部分Start */
        ...
        rows.forEach((x) => {
          ...
          if (index === -1) {
            if (x.externalPartNoList) {
              this.$set(x, 'externalPartNoList', x.externalPartNoList)
            }
          }
        })
    ...
    async submitForm(submitType, isConfirm = true, gotoRoute) {
      ...
      if (valid1) {
        ...
        // }
        this.collapseWarningForProductInfo = false
        param.purchaseRequisiteDetailList = JSON.parse(JSON.stringify(detailList))
    ...
    /* 版本比对 */
    revisionComparison() {
      this.$refs.RevisionComparisonDlg.handleOpen(this.form.purchaseRequisiteId)
    },
    exitComparison() {
      this.isComparison = false
    ...
    queryProductHistoryVersion() {
      ...
      queryPRHistoryVersion({ purchaseRequisiteId: this.rowId }).then((res) => {
    ...
    aplSubmitSuccess() {
      const vm = this
      vm.$message.success(
        `${vm.$t('ui.approvedSuccess')}`.replace('$1', `[${vm.form.purchaseRequisiteNo}]`)
      )
    ...
    rjSubmitSuccess() {
      const vm = this
      vm.$message.success(
        `${vm.$t('ui.rejectedSuccess')}`.replace('$1', `[${vm.form.purchaseRequisiteNo}]`)
      )
    ...
    cancelSubmitSuccess() {
      const vm = this
      vm.$message.success(
        `${vm.$t('ui.cancelledSuccess')}`.replace('$1', `[${vm.form.purchaseRequisiteNo}]`)
      )
    ...
    closeSubmitSuccess() {
      const vm = this
      vm.$message.success(
        `${vm.$t('ui.closedSuccess')}`.replace('$1', `[${vm.form.purchaseRequisiteNo}]`)
      )

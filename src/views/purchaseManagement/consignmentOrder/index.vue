<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <template v-if="item.prop === 'businessPartnerName'">
            <DocumentNoLink
      ...
      <el-table-column
        ...
        v-if="
          '4,5,6,7'.indexOf(activeName) !== -1 &&
          checkPermi(['purchaseManagement:consignmentOrder:attachment'])
        "
      >
        <template slot-scope="scope">
          <div class="flexCen">
            <!--  :class="{ flexCen: '1,2,3,7,8'.indexOf(activeName) !== -1 }"
            ...
            <!-- <svg-icon
              ...
              class="primary-pointer"
              style="font-size: 20px"
              v-if="'0,4,5,6'.indexOf(activeName) !== -1"
    ...
    <uploadDlg ref="uploadDlg" @onSuccess="getList" />
  </div>
</template>
...
import SendPODlg from '@/views/purchaseManagement/consignmentOrder/sendPODlg.vue'
import uploadDlg from '@/views/purchaseManagement/consignmentOrder/uploadDlg.vue'
export default {
  name: 'ConsignmentOrder',
  mixins: [pageMixin, tableDataMixin],
  components: { systemOperationLogTableDlg, ToolTipShowListForFN, SendPODlg, uploadDlg },
  data() {
    return {
      saveKey: '1',
      // 遮罩层
  ...
  computed: {
    addAuth() {
      return this.checkPermi(['purchaseManagement:consignmentOrder:add'])
    },
    sendAuth() {
      return this.checkPermi(['purchaseManagement:consignmentOrder:sendPO'])
    },
    exportAuth() {
      return this.checkPermi(['purchaseManagement:consignmentOrder:export'])
    },
    curColumns() {
  ...
  async created() {
    ...
    const a = this.MyDictDataClass.getDictFn('p_consignment_order_status')
    ...
    this.searchData0.forEach((item) => {
      if (item.name === 'consignmentOrderStatusList') {
  ...
  methods: {
    ...
    initQueryParams(isResetDate = true) {
      ...
      this.queryParams.selectId = 'conditionForConsignmentOrderNo'
      this.queryParams.selectId2 = 'createdIdList'
      this.searchData.forEach((item) => {
        if (item.name === 'selectAnInputName') {
          item.selectId = 'conditionForConsignmentOrderNo'
          item.inputVal = ''
        }
    ...
    tableRowClassName({ row, rowIndex }) {
      ...
      for (const item of this.ids.values()) {
        if (item === row.consignmentOrderId) {
    ...
    getList(query) {
      ...
      queryConsignmentOrderList(params).then((response) => {
        ...
        rows.forEach((item) => {
          ...
          item.consignmentOrderStatusStr = this.selectDictLabel(
            this.statusDicts,
            item.consignmentOrderStatus
          )
    ...
    querySearchList() {
      querySearchList({ labelType: this.activeName }).then((res) => {
        ...
        this.conditionForConsignmentOrderNoList = (
          data.conditionForConsignmentOrderNoList || []
        ).map((item) => {
          return {
            value: item,
            label: item
    ...
    handleUpload(row) {
      this.$refs.uploadDlg.handleOpen(row)
    },
    /** 搜索 */
    handleSearchForm() {
    ...
    setFuzzyData() {
      this.searchData.forEach((item) => {
        if (item.name === 'selectAnInputName') {
          if (item.selectId === 'conditionForConsignmentOrderNo') {
            this.$set(item, 'fuzzyData', this.conditionForConsignmentOrderNoList)
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.consignmentOrderId)
    ...
    handleAdd() {
      this.$router.push({
        path: '/purchaseManagement/addConsignmentOrder',
        query: {
          timeId: Date.now()
        }
      })
    ...
    handleUpdate(row, column) {
      ...
      this.$router.push({
        path: '/purchaseManagement/editConsignmentOrder',
        query: {
          id: row.consignmentOrderId,
          timeId: Date.now()
        }
      })
    },
    ...
    handleOperationLog(row) {
      this.$refs.systemOperationLogTableDlg.handleOpen(queryOperationLog, {
    ...
    handleExport(data) {
      ...
      if (exportType === 2) {
        params.exportIdList = vm.tableList.map((item) => item.consignmentOrderId)
      }
      if (exportType === 3) {
      ...
      this.$importOrExportDownFile(
        '/purchase/consignmentOrder/export',
        params,
        `${this.getFileNameDate('Consignment Order')}.xlsx`
      )
        .then((res) => {

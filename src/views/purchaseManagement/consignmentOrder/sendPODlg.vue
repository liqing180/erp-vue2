      ...
      >
        ...
        >
          <template slot="header" slot-scope="{ column }">
            <span v-if="['consignmentOrderNo', 'carrier'].includes(item.prop)">
              <span style="color: #ff4949; margin-right: 4px">*</span>{{ column.label }}
            </span>
            <span v-else>{{ column.label }}</span>
          </template>
          <template slot-scope="scope">
            <template v-if="item.prop === 'consignmentOrderNo'">
              <el-select
                style="width: 98%"
                v-model="scope.row.consignmentOrderNo"
                placeholder=""
                v-if="!comDisFrom"
              ...
              >
                <el-option
                  ...
                  :key="item.consignmentOrderNo"
                  :label="item.consignmentOrderNo"
                  :value="item.consignmentOrderNo"
                >
                </el-option>
              </el-select>
              <template v-else>{{ scope.row.consignmentOrderNo }}</template>
            </template>
    ...
    <div class="mt20" v-if="form.isSecond === '1'">
      <el-form ref="form" :model="form" :rules="rules" label-width="140px" :disabled="comDisFrom">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item :label="`${$t('PURCHASE.billsOfLadingNo')}`" prop="carrier">
...
import {
  sendConsignmentOrder,
  queryCanSendConsignmentOrderList,
  queryConsignmentOrderSendMsg
...
export default {
      ...
      columns: [
        {
          ...
          label: vm.$t('PURCHASE.consignmentOrderNo'),
          visible: true,
          minWidth: 200,
          tooltip: true
        },
  ...
  methods: {
    ...
    handleEdit(row) {
      ...
      queryConsignmentOrderSendMsg({ consignmentOrderId: row.consignmentOrderId }).then((res) => {
        console.log(res)
        const data = res.data || {}
        this.form = data
    ...
    queryCanSendConsignmentOrderList() {
      queryCanSendConsignmentOrderList({ menuPerms: this.menuKey.consignmentOrder }).then((res) => {
        const data = res.data || []
        this.poRowList = data
        console.log(res)
      })
    ...
    purchaseOrderNoChange(row) {
      const curSelect = this.poRowList.find(
        (item) => item.consignmentOrderNo === row.consignmentOrderNo
      )
      if (curSelect) {
        this.$set(row, 'consignmentOrderId', curSelect.consignmentOrderId)
    ...
    async submitForm() {
      ...
      if (valid) {
        if (isSecond === '1') {
          ...
          this.form.commonFileList = myFileIds1
        }
        ...
        this.$modal
          ...
          .then(() => {
            return sendConsignmentOrder(param)
          })
          .then((response) => {
    ...
    validProductList(detailList) {
      ...
      const purchaseOrderNoReq = detailList.find((item) => {
        return !this.$resultOfBoolean(item.consignmentOrderNo)
      })
      if (purchaseOrderNoReq) {
        this.$modal.msgError(
          this.$t('ui.commonReqMsg').replace('$1', this.$t('PURCHASE.consignmentOrderNo'))
        )
        return valid
      }

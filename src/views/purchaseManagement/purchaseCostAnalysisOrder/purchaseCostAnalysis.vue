  ...
  <div class="app-container">
    ...
    <el-row :gutter="10">
      <el-col :span="1.5">
        <el-button
          ...
          @click="openExport"
          v-hasPermi="['purchaseManagement:purchaseCostAnalysisOrder:export']"
    ...
    >
      <el-table-column
        ...
        align="center"
        v-if="checkPermi(['purchaseManagement:purchaseCostAnalysisOrder:export'])"
      >
      </el-table-column>
      <el-table-column type="index" :label="$t('ui.sn')" width="60" fixed="left" align="center">
        <template slot-scope="scope">
          <span>{{
            scope.row.stockLogId
...
import {
  queryPurchaseCostDocumentList,
  querySearchList
} from '@/api/purchaseManagement/purchaseCostAnalysisOrder'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.loading = true
      queryPurchaseCostDocumentList(param)
        .then((response) => {
          ...
          rows.forEach((item) => {
            ...
            item.purchaseDepositAmount = this.$numberStr(item.purchaseDepositAmount, 2)
          ...
          if (rows.length > 0 && response.summary) {
            ...
            summary.purchaseDepositAmount = this.$numberStr(summary.purchaseDepositAmount, 2)
    ...
    selectable(row, rowIndex) {
      if (!row.stockLogId) return false
      return true // 不禁用
    },
    nav(row, type) {
    ...
    handleExport(data) {
      ...
      this.$importOrExportDownFile(
        '/inventory/purchaseCostDocument/exportPurchaseCostDocumentList',
        params,
        `${this.getFileNameDate('Purchase Cost Analysis (Order)')}.xlsx`
      )
        .then((res) => {

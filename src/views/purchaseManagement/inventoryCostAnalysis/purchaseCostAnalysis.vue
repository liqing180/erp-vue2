    ...
    <el-row :gutter="10">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleImport"
          v-hasPermi="['purchaseManagement:inventoryCostAnalysis:import']"
          >{{ $t('menu.importPurchaseCost') }}</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    >
      ...
      <el-table-column type="index" :label="$t('ui.sn')" width="60" fixed="left" align="center">
        <template slot-scope="scope">
          <span>{{
            scope.row.purchaseCostProductId
      ...
      >
        <template slot-scope="scope">
          <div class="flexCen">
            <svg-icon
              v-if="scope.row.purchaseCostProductId"
              icon-class="list2"
...
import productDetailChartDlg from './components/productDetailChartDlg.vue'
import {
  queryPurchaseCostProductList,
  queryPurchaseCostProductSearchList
...
export default {
  ...
  dicts: [],
  mixins: [pageMixin],
  components: { stockOutOrStockInDlg, productDetail, productDetailChartDlg },
  data() {
    const vm = this
    return {
      saveKey: '1',
      ...
      importOptions: {
        // 下载模板
        downFileUrl: '/inventory/purchaseCostProduct/exportImportTemplateForUpdateCost',
        // 上传校验
        uploadUrl:
          process.env.VUE_APP_BASE_API +
          '/inventory/purchaseCostProduct/checkImportTemplateDataForUpdateCost',
        // 提交导入
        importUrl: '/inventory/purchaseCostProduct/importTemplateDataForUpdateCost',
        // 下载文件名
        fileName: 'Purchase Cost Analysis'
      }
    }
  },
  watch: {},
  ...
  methods: {
    ...
    getList() {
      this.queryPurchaseCostProductSearchList()
      ...
      this.loading = true
      queryPurchaseCostProductList(param)
        .then((response) => {
          ...
          if (rows.length > 0 && response.summary) {
            ...
            summary.qty = this.$numberStr(summary.qty, 3)
            rows.push(summary)
          }
          this.tableList = rows
          this.loading = false
    ...
    // 搜索下拉值
    queryPurchaseCostProductSearchList() {
      queryPurchaseCostProductSearchList({}).then((res) => {
        const companyName = res.data.legalEntityName
        const conditionForBrandList = res.data.brandList || []

        this.queryParams.companyName = companyName
    ...
    handleDetail(row) {
      if (!row.purchaseCostProductId) return
      this.$refs.productDetail.handleOpen(row)
    },
    tableRowClassName({ row, rowIndex }) {
      let color = ''
      for (const item of this.ids.values()) {
        if (item === row.purchaseCostProductId) {
    ...
    selectable(row, rowIndex) {
      if (!row.purchaseCostProductId) return false
      return true // 不禁用
    },
    nav(row, type) {
    ...
    /** 重置 */
    resetSearchForm() {
      const { pageSize, companyName } = this.queryParams
      this.queryParams = { pageNum: 1, pageSize, companyName }
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.purchaseCostProductId)
    ...
    handleExport(data) {
      ...
      if (exportType === 2) {
        params.exportIdList = vm.tableList.map((item) => item.purchaseCostProductId)
      ...
      this.$importOrExportDownFile(
        '/inventory/purchaseCostProduct/exportPurchaseCostProductList',
        params,
        `${this.getFileNameDate('Purchase Cost Analysis')}.xlsx`
      )
        .then((res) => {

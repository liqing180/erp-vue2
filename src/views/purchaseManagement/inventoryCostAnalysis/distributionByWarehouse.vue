    ...
    >
      ...
      <el-table-column type="index" :label="$t('ui.sn')" width="60" fixed="left" align="center">
        <template slot-scope="scope">
          <span>{{
            scope.row.purchaseCostProductWarehouseId
      ...
      >
        <template slot-scope="scope">
          <div class="flexCen">
            <svg-icon
              v-if="scope.row.purchaseCostProductWarehouseId"
              icon-class="list2"
...
import stockOutOrStockInDlg from './componentsWarehouse/stockOutOrStockInDlg.vue'
import productDetail from './componentsWarehouse/productDetail.vue'
import productDetailChartDlg from './componentsWarehouse/productDetailChartDlg.vue'

import {
  queryPurchaseCostProductWarehouseList,
  queryPurchaseCostProductWarehouseSearchList
} from '@/api/purchaseManagement/distributionByWarehouse'
export default {
  ...
  methods: {
    ...
    getList() {
      this.queryPurchaseCostProductWarehouseSearchList()
      ...
      this.loading = true
      queryPurchaseCostProductWarehouseList(param)
        .then((response) => {
    ...
    // 搜索下拉值
    queryPurchaseCostProductWarehouseSearchList() {
      queryPurchaseCostProductWarehouseSearchList({}).then((res) => {
    ...
    handleDetail(row) {
      if (!row.purchaseCostProductWarehouseId) return
      this.$refs.productDetail.handleOpen(row)
    },
    tableRowClassName({ row, rowIndex }) {
      let color = ''
      for (const item of this.ids.values()) {
        if (item === row.purchaseCostProductWarehouseId) {
    ...
    selectable(row, rowIndex) {
      if (!row.purchaseCostProductWarehouseId) return false
      return true // 不禁用
    },
    nav(row, type) {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.purchaseCostProductWarehouseId)
    ...
    handleExport(data) {
      ...
      if (exportType === 2) {
        params.exportIdList = vm.tableList.map((item) => item.purchaseCostProductWarehouseId)
      ...
      this.$importOrExportDownFile(
        '/inventory/purchaseCostProductWarehouse/exportPurchaseCostProductWarehouseList',
        params,

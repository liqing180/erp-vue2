  ...
  >
    <div style="display: flex">
      <el-radio-group v-model="type" size="medium" class="radio-card" @input="radioChange">
        <el-radio-button label="imageForOrder">{{ $t('PURCHASE.order') }}</el-radio-button>
        <el-radio-button label="imageForVendor">{{ $t('PURCHASE.supplier') }}</el-radio-button>
        <el-radio-button label="imageForCountry">{{ $t('ui.country') }}</el-radio-button>
      </el-radio-group>
    </div>
...
<script>
import { queryImageForProductDocument } from '@/api/purchaseManagement/distributionByWarehouse'
...
export default {
  ...
  methods: {
    openChart(row = {}) {
      ...
      this.$set(this.queryParams, 'warehouseId', warehouseId || undefined)
      this.$set(this.queryParams, 'startDate', startDate || undefined)
      this.$set(this.queryParams, 'endDate', endDate || undefined)
      this.$set(this.queryParams, 'countryIdList', countryIdList || undefined)
      this.$set(this.queryParams, 'businessPartnerIdList', businessPartnerIdList || undefined)
      this.$set(this.queryParams, 'pageNum', pageNum || undefined)
      this.$set(this.queryParams, 'pageSize', pageSize || undefined)
      this.queryImageForProductDocument()
    },
    queryImageForProductDocument() {
      queryImageForProductDocument(this.queryParams).then((res) => {
        this.form = res.data || {}
        this.$nextTick(() => {
    ...
    handler(row) {
      ...
      if (this.type === 'imageForOrder') {
        xAxisData = imageXList.map((x) => x.documentNo)
      } else if (this.type === 'imageForVendor') {
        xAxisData = imageXList.map((x) => x.businessPartnerName)
      } else if (this.type === 'imageForCountry') {
        xAxisData = imageXList.map((x) => x.country)
      }
      ...
      const yAxisData = [
        ...
        },
        {
          ...
          minInterval: 1,
          min: 0,
          max: maxNum === 0 || isNaN(maxNum) ? 100 : undefined,
          axisLine: {
            show: true
          },
          axisLabel: {

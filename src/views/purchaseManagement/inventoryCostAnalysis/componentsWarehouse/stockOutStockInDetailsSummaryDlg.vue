<template>
  <el-dialog
    v-dialogDrag
    v-dialogDragWidth
    :title="$t('PURCHASE.StockOutStockInDetailsSummary')"
...
import { inventoryQuerySummaryListByProductDocument } from '@/api/reportManagement/inventoryCostAnalysis'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      columns: [
        ...
        {
          prop: 'unqualifiedQtyForDocument',
        ...
        {
          prop: 'inventoryAmountForDocument',
  ...
  methods: {
    ...
    getList() {
      ...
      inventoryQuerySummaryListByProductDocument(params)
        .then((response) => {
          const rows = response.rows || []
          rows.forEach((item) => {
            item.createdTime = this.parseTime(item.createdTime, this.fmtForYmdhms)
            item.modifiedTime = this.parseTime(item.modifiedTime, this.fmtForYmdhms)
            item.qtyOnHandForDocument = this.$numberStr(
              item.qtyOnHandForDocument,
              this.$getDecNum(item.decimalNum)
            )
            item.unqualifiedQtyForDocument = this.$numberStr(
              item.unqualifiedQtyForDocument,
              this.$getDecNum(item.decimalNum)
            )
            ...
            item.inventoryCostForDocument = this.$numberStr(item.inventoryCostForDocument, 2)
            item.inventoryAmountForDocument = this.$numberStr(item.inventoryAmountForDocument, 2)

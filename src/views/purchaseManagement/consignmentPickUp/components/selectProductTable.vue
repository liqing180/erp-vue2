...
import { queryCanSelectProductList } from '@/api/purchaseManagement/consignmentPickUp'
import { queryCanSelectServiceList } from '@/api/projectManagement/projectTask'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      let queryFn
      if (this.activeName === '1') {
        param.alreadySelectProductIdList = this.alreadyProductIdList
          .filter((x) => x.productType === '1')
          .map((x) => x[this.rowIdKey])
        queryFn = queryCanSelectProductList
      } else {
        param.alreadySelectProductIdList = this.alreadyProductIdList
          .filter((x) => x.productType === '2')
          .map((x) => x[this.rowIdKey])
        param.purpose = '1'
        queryFn = queryCanSelectServiceList
      }

    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          @click="handleAddBtn"
          v-if="addBtnShow"
          :disabled="!formData.businessPartnerId || !formData.warehouseId"
          type="primary"
          icon="el-icon-plus"
          size="mini"
      ...
      <div class="footer-content">
        <div>{{ $t('PURCHASE.totalAmount') }} : {{ $numberStr(comTotalAmount, 2) }}</div>
        <div class="ml20">{{ $t('PURCHASE.totalQTY') }} : {{ comTotalQty }}</div>
      </div>
    </el-row>
    <el-table
      border
    ...
    >
      ...
      >
        <template slot="header" slot-scope="{ column }">
          <span v-if="['pickupQty', 'expectedDeliveryDate'].includes(item.prop)">
        ...
        <template slot-scope="scope">
          ...
          <template v-else-if="item.prop === 'expectedDeliveryDate'">
            <el-date-picker
              v-if="!comDisFrom"
...
export default {
  ...
  watch: {
    ...
    'formData.consignmentPickupDetailList': {
      immediate: true,
      handler: function (selected) {
        this.tableList = this.formData.consignmentPickupDetailList
  ...
  data() {
    ...
    return {
      saveKey: '18',
      savePath: 'consignmentPickUpTable',
      activeName: '1',
      columns: [
        {
          prop: 'productName',
  ...
  methods: {
      ...
      this.columns.forEach((item) => {
        ...
        switch (prop) {
          ...
          case 'expectedDeliveryDate':
            item.fixedWidth = !vm.comDisFrom ? 180 : undefined
            break
          case 'remarks':
    ...
    getLineTotal(row) {
      this.$nextTick(() => {
        if (this.$resultOfBoolean(row.pickupQty) && this.$resultOfBoolean(row.consignedCost)) {
          const lineTotal = row.consignedCost * row.pickupQty
    ...
    changeExpectedDeliveryDate(index) {
      if (index === 0) {
        ...
        if (topRow && topRow.expectedDeliveryDate) {
          const oldCommittedDate = this.tableList.find(
            (item, index) => index > 0 && item.expectedDeliveryDate
          )
          if (oldCommittedDate) {
            const msg = this.parseTime(topRow.expectedDeliveryDate, this.fmtForYmd)
            this.$modal
              .confirm(this.$t('PURCHASE.committedDateConfirm').replace('$1', msg))
              .then(() => {
                this.$set(this.totalForm, 'committedDate', topRow.expectedDeliveryDate)
                this.tableList.forEach((row) => {
                  this.$set(row, 'expectedDeliveryDate', topRow.expectedDeliveryDate)
                })
              })
          } else {
            this.tableList.forEach((row) => {
      ...
      const committedDateList = this.tableList.map((item) => item.expectedDeliveryDate || 0)

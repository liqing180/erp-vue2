<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <template v-if="['businessPartnerName'].includes(item.prop)">
      ...
      >
        <template slot-scope="scope">
          <div
            class="flexSb"
            :class="{ flexCen: '1,2,3'.indexOf(activeName) !== -1 }"
...
export default {
  ...
  async created() {
    ...
    const a = this.MyDictDataClass.getDictFn('p_consignment_pickup_status')
    ...
    this.searchData0.forEach((item) => {
      if (item.name === 'consignmentPickupStatusList') {
  ...
  methods: {
    initQueryParams() {
      ...
      this.queryParams.selectId = 'conditionForConsignmentPickupNo'
      this.queryParams.selectId2 = 'createdIdList'
      this.searchData.forEach((item) => {
        if (item.name === 'selectAnInputName') {
          item.selectId = 'conditionForConsignmentPickupNo'
          item.inputVal = ''
        }
    ...
    tableRowClassName({ row, rowIndex }) {
      ...
      for (const item of this.ids.values()) {
        if (item === row.consignmentPickupId) {
    ...
    getList(query) {
      ...
      queryConsignmentPickupList(params).then((response) => {
        ...
        rows.forEach((item) => {
          ...
          item.consignmentPickupStatusStr = this.selectDictLabel(
            this.statusDicts,
            item.consignmentPickupStatus
          )
    ...
    querySearchList() {
      querySearchList({ labelType: this.activeName }).then((res) => {
        ...
        this.conditionForConsignmentPickupNoList = (
          data.conditionForConsignmentPickupNoList || []
        ).map((item) => {
          return {
            value: item,
            label: item
    ...
    setFuzzyData() {
      this.searchData.forEach((item) => {
        if (item.name === 'selectAnInputName') {
          if (item.selectId === 'conditionForConsignmentPickupNo') {
            this.$set(item, 'fuzzyData', this.conditionForConsignmentPickupNoList)
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.consignmentPickupId)
    ...
    handleAdd() {
      this.$router.push({
        path: '/purchaseManagement/addConsignmentPickUp',
        query: {
          timeId: Date.now()
        }
      })
    ...
    handleUpdate(row, column) {
      ...
      this.$router.push({
        path: '/purchaseManagement/editConsignmentPickUp',
        query: {
          id: row.consignmentPickupId,
          timeId: Date.now()
        }
      })
    },
    ...
    handleOperationLog(row) {
      this.$refs.systemOperationLogTableDlg.handleOpen(queryOperationLog, {
        consignmentPickupId: row.consignmentPickupId
      })
    },
    /** 导出按钮操作 */
    /** 导出按钮操作 */
    ...
    handleExport(data) {
      ...
      if (exportType === 2) {
        params.exportIdList = vm.tableList.map((item) => item.consignmentPickupId)
      }
      if (exportType === 3) {
      ...
      this.$importOrExportDownFile(
        '/purchase/consignmentPickup/export',
        params,
        `${this.getFileNameDate('Consignment Pickup')}.xlsx`
      )
        .then((res) => {

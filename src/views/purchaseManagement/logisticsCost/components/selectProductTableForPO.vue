        ...
        >
          ...
          <template slot-scope="scope">
            <template v-if="item.prop === 'qty'">
              <!-- disabled dropShipping === '1' 直运的时候数量不让编辑
                https://www.tapd.cn/tapd_fe/53232180/bug/detail/1153232180001032116
              -->
              <el-input-number
              ...
              />
              <!-- :disabled="
                  scope.row.dropShipping === '1' || scope.row.logisticsQtyIsCanUpdate !== '1'
                " -->
              <template v-else>{{
            ...
            <template v-else-if="item.prop === 'purchaseQtyForBasic'">
              <template>{{ $numberStr(scope.row[item.prop], scope.row.decimalNum) }}</template>
            </template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      columns: [
        ...
        /* {
          prop: 'businessGroupBasicUnitPrice',
  ...
  computed: {
    ...
    legalEntityInfo() {
      ...
      // currencyCode: legalEntityInfo.currencyCode,
    },
    fmtForYmdhms() {
  ...
  methods: {
    ...
    querySearchFromLCReceivedNoteDetailBatchList() {
      ...
      querySearchFromLCReceivedNoteDetailBatchList(param).then((res) => {
        const data = res.data || {}
        const conditionForPurchaseOrderNoList = (data.conditionForPurchaseOrderNoList || []).map(
          (item) => {
            return {
              value: item,
              label: item
        ...
        this.searchData[1].fuzzyData = conditionForPurchaseOrderNoList

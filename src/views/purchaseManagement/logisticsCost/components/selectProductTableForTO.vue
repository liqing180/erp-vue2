  ...
  data() {
    ...
    return {
      ...
      columns: [
        {
          prop: 'documentNo',
          label: vm.$t('PURCHASE.transferOrderNo'),
          visible: true,
          minWidth: 200,
          sortable: 'custom',
          tooltip: true,
      ...
      searchData: [
        ...
        {
          name: 'conditionForTransferOrderNo',
          label: this.$t('PURCHASE.transferOrderNo'),
  ...
  methods: {
    ...
    querySearchFromLCReceivedNoteDetailBatchList() {
      ...
      querySearchFromLCReceivedNoteDetailBatchList(param).then((res) => {
        const data = res.data || {}
        const conditionForTransferOrderNoList = (data.conditionForTransferOrderNoList || []).map(
          (item) => {
            return {
              value: item,
              label: item
        ...
        this.searchData[1].fuzzyData = conditionForTransferOrderNoList
      })
    },
    getList() {
      ...
      queryFn(param)
        .then((response) => {
          ...
          rows.forEach((item) => {
            ...
            // item.qty = item.logisticsLineQty
            // item.lineTotal = item.logisticsLineTotal
            // item.unitPrice = item.averageUnitPriceForBasic
            // item.documentId = item.transferOrderId
            // item.documentNo = item.transferOrderNo
            // item.documentDetailId = item.transferOrderProductId
          })

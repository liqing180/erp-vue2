  ...
  data() {
    ...
    return {
      ...
      columns: [
        {
          prop: 'documentNo',
          label: vm.$t('PURCHASE.salesGoodsReturnNo'),
          visible: true,
          minWidth: 200,
          tooltip: true,
          fixed: true
      ...
      searchData: [
        ...
        },
        {
          ...
          label: this.$t('PURCHASE.salesGoodsReturnNo'),
        ...
        },
        {
          ...
          label: this.$t('PURCHASE.customerName'),
          type: 'AutocompleteEle',
          fuzzyData: [],
          defaultProps: {
            value: 'value',
  ...
  methods: {
    ...
    querySearchFromLCReceivedNoteDetailBatchList() {
      ...
      querySearchFromLCReceivedNoteDetailBatchList(param).then((res) => {
        const data = res.data || {}
        const conditionForSalesGoodsReturnNoList = (
        ...
        this.searchData[1].fuzzyData = conditionForSalesGoodsReturnNoList
      ...
      queryFn(param)
        .then((response) => {
          ...
          rows.forEach((item) => {
            ...
            item.rmaTypeStr =
              item.rmaTypeShowStr ||
              this.selectDictLabel(this.dict.type.sales_rma_type, item.rmaType)
            // item.documentId = item.salesGoodsReturnId
            // item.documentNo = item.salesGoodsReturnNo
            // item.documentDetailId = item.salesGoodsReturnProductId
          })
    ...
    onDictReady() {
      ...
      //   item.rmaTypeStr = this.selectDictLabel(this.dict.type.sales_rma_type, item.rmaType)
      // })
      this.$$getColumnContentMaxWidth(this.columns, this.tableList)
    },
    qtyChange(row) {

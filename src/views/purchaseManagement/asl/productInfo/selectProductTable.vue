...
export default {
  ...
  computed: {
    ...
    comSelectedNum() {
      const selectedRows = (this.activeName === '1' ? this.selectList1 : this.selectList2).filter(
        (item) => !item.purchaseQuotationDetailId
      )
  ...
  methods: {
    ...
    handleAdd(data = [], alreadyProductIdList) {
    ...
    getList() {
      ...
      param.selectedIdList = this.$$getSelectedIdList(selectedRows, this.rowIdKey)

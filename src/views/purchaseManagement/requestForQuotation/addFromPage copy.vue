        ...
        <el-collapse v-model="activeNames">
          <div class="form-card">
            <el-collapse-item name="1">
              ...
              <!-- 正常询价 -->
              <template v-if="mergeForm.isMergePrAddress !== '1'">
                <el-form
                  ref="form1"
                  :model="form"
                    ...
                    <el-col :span="8">
                      <el-form-item :label="`${$t('PURCHASE.dropShipping')}`">
                        <el-switch
                          ...
                          :disabled="form.isSalesDropShipping === '1' || form.rfqType === '2'"
                        ></el-switch>
                        <!-- form.rfqType === '2' ||  -->
                      </el-form-item>
                    </el-col>
...
export default {
  ...
  methods: {
    ...
    dropShippingChange() {
      ...
      if (
        this.initForm &&
        (this.initForm.rfqType === '3' || this.initForm.rfqType === '4') &&
    ...
    checkProductIsAlreadyRFQ(list) {
      ...
      checkProductIsAlreadyRFQ({ rfqDetailList: arr }).then((res) => {
        ...
        this.rfqDetailList.forEach((item) => {
          if (item.externalPartNoListJson) {
            this.$set(item, 'externalPartNoListJson', JSON.parse(item.externalPartNoListJson))
          } else {

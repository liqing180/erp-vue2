<template>
  ...
  >
    ...
    >
      ...
      >
        <el-form-item
          :label="`${$t('PURCHASE.RequiredQuotes')}`"
          prop="inquirySupplierQuantity"
          label-width="120px"
        >
          <el-input-number
            style="width: 100%"
...
export default {
  ...
  methods: {
    ...
    handleAdd(alreadySelectIdList = [], inquirySupplierQuantity) {
      this.resetForm('form')
    ...
    getSystemSetup() {
      getSystemSetup().then((res) => {
        console.log(res, '=================305')
        const { inquirySupplierQuantity } = res.data
        this.inquirySupplierQuantity = inquirySupplierQuantity
        if (!this.form.inquirySupplierQuantity) {
          this.$set(this.form, 'inquirySupplierQuantity', inquirySupplierQuantity)
        }
      })
    },
    ...
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          this.$emit('onSuccess', this.selectList, this.form.inquirySupplierQuantity)
          this.visible = false
        }
      })
    }

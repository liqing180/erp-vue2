  ...
  >
    ...
    <div class="mt20" style="display: flex; align-items: flex-start" v-loading="loading">
      ...
      <div class="w100" style="overflow: hidden">
        ...
        <pagination
          :selectedNum="activeName === '1' ? selectList1.length : selectList2.length"
          :saveKey="saveKey"
          :savePath="savePath"
          v-show="total > 0"
          :total="total"
...
export default {
  ...
  methods: {
    ...
    handleAdd(data = [], type = '') {
      ...
      // this.selectList1 = data.filter((x) => x.productType !== '2')
      // this.selectList2 = data.filter((x) => x.productType === '2' && x[this.rowIdKey])
      // this.queryParams.productCategoryId = ''
    ...
    getList() {
      ...
      if (this.activeName === '1') {
        ...
        //   .filter((x) => x.productType === '1' && x[this.rowIdKey])
        //   .map((x) => x[this.rowIdKey])
        if (this.type === 'supplier') {
          param.businessPartnerIdList = (this.rfqVendorList || []).map(
          ...
          delete param.businessPartnerIdList
        }
        queryFn = queryRFQCanSelectProductList
      } else {
        // param.alreadyProductIdList = this.alreadyProductIdList
        //   .filter((x) => x.productType === '2' && x[this.rowIdKey])
        //   .map((x) => x[this.rowIdKey])
        param.purpose = '1'

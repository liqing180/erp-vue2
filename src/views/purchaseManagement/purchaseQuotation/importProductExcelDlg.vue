      ...
      <el-row>
        <el-form-item :label="$t('ui.uploadData')" prop="file">
          <el-upload
            ...
            :data="{ purchaseQuotationId: purchaseQuotationId }"
...
<script>
import { importPurchaseQuotationProduct } from '@/api/purchaseManagement/purchaseQuotation'
  ...
  data() {
    return {
      ...
      importOptions: {
        isUploading: false,
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + '/purchase/purchaseQuotation/checkPQImportTemplate'
      },
      fileList: [],
      updateSupport: 0,
  ...
  methods: {
    ...
    downLoad() {
      ...
      this.$importOrExportDownFile(
        '/purchase/purchaseQuotation/exportPQImportTemplate',
        {
          purchaseQuotationId: this.purchaseQuotationId,
          ...this.formData
        },
        `${this.getFileNameDate('PQ Product Info')}.xlsx`
      )
        .then((res) => {
            ...
            .then((res) => {
              vm.$modal
                ...
                .then(() => {
                  ...
                  formData.append('purchaseQuotationId', vm.purchaseQuotationId)
                  return importPurchaseQuotationProduct({ formData })
                })
                .then((response) => {
                  vm.btnLoading = false
                  ...
                  this.$emit('onSuccess', data.purchaseQuotationDetailList || [])
                })
                .catch((err) => {
                  vm.btnLoading = false

            ...
            <el-collapse-item name="1">
              ...
              >
                ...
                <el-row>
                  ...
                  <el-col :span="8">
                    <el-form-item :label="`${$t('PURCHASE.taxAmount')}`">
                      <el-input-number
...
export default {
  ...
  computed: {
    ...
    createPOBtnShow() {
      return (
        this.isView &&
        this.form.isCanGoToPO === '1' &&
        this.checkPermi(['purchaseManagement:purchaseOrder:add'])
      )
    }
  },
  beforeCreate() {
  ...
  methods: {
    handleCreatePO() {
      const fromSrcPath = encodeURIComponent('/purchaseManagement/viewPurchaseQuotation')
      this.$router.push({
    ...
    async handleUpdate() {
      ...
      queryPurchaseQuotationById({ purchaseQuotationId: rowId }).then((res) => {
        ...
        list.forEach((item) => {
          ...
          this.$set(item, 'taxAmountStr', this.$numberStr(item.taxAmount, 6, false, 2))
          if (!this.comDisFrom && item.pqResult === '3') {
            this.$set(item, 'pqResult', '1')
          }
        })
      ...
      if (valid1) {
        ...
        this.$modal
          ...
          .then(() => {
            ...
            return revisePurchaseQuotation(param)
          })
          .then((response) => {

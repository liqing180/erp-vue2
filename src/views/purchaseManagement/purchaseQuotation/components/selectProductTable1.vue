      ...
      >
        <template slot-scope="scope">
          <template v-if="item.prop === 'isActive'">
            ...
            <el-tag v-else>{{ $t('uiBtn.active') }}</el-tag>
          </template>
          ...
          <template v-else-if="item.prop === 'valuationAmount'">{{
            $numberStr(scope.row.valuationAmount, 6, false, 2)
          }}</template>
...
import { queryCanSelectProductList } from '@/api/purchaseManagement/purchaseQuotation'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      if (this.purchaseQuotationDataId) {
        param.purchaseQuotationDataId = this.purchaseQuotationDataId
      } else {
        param.purchaseQuotationDataId = ''
      }
      param.rfqId = this.rfqId
          ...
          this.selectList1.forEach((item) => {
            rows.forEach((row) => {
              if (row[this.rowIdKey] === item[this.rowIdKey]) {
                row.isCanDelete = item.isCanDelete
              }
            })
          })
    ...
    selectable(row, rowIndex) {
      if (row.isCanDelete && row.isCanDelete !== '1') {
        return false // 禁用
      }
      return true // 不禁用
    },

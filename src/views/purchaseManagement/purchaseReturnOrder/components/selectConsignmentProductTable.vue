...
import { queryProductListForConsignmentOrder } from '@/api/purchaseManagement/purchaseReturnOrder'
...
export default {
  ...
  computed: {
    ...
    showAllAuth() {
      return this.checkPermi(['purchaseManagement:purchaseReturnOrder:departmentData'])
    }
  },
  watch: {},
  created() {
  ...
  methods: {
    ...
    handleAdd(data = []) {
      this.alreadyProductIdList = data.map((x) => x.productId) || []
    ...
    getList() {
      ...
      param.consignmentOrderId = this.formData.consignmentOrderId
      param.warehouseId = this.formData.warehouseId
      param.purchaseReturnId = this.formData.purchaseReturnId
      param.selectedIdList = this.$$getSelectedIdList(this.selectIds1, this.rowIdKey)
      if (this.alreadyProductIdList.length > 0) {
      ...
      this.loading = true
      queryProductListForConsignmentOrder(param)
        .then((response) => {
          this.loading = false

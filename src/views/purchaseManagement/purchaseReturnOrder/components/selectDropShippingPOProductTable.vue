...
import { queryDropShippingCanSelectProductList } from '@/api/purchaseManagement/purchaseReturnOrder'
...
export default {
  ...
  methods: {
    ...
    handleAdd(data = []) {
      // this.alreadyProductIdList = data.map((x) => x.deliveryOrderProductId) || []
    ...
    getList() {
      ...
      // }
      this.loading = true
      queryDropShippingCanSelectProductList(param)
        .then((response) => {
          this.loading = false

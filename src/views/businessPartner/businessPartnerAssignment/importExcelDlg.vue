...
<script>
import { importData } from '@/api/businessPartner/bpAssignment'
  ...
  data() {
    return {
      ...
      importOptions: {
        isUploading: false,
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + '/system/bpAssignment/checkImportData'
      },
      fileList: [],
      updateSupport: 0,
  ...
  methods: {
    ...
    downLoad() {
      ...
      this.$importOrExportDownFile(
        '/system/bpAssignment/downTemplate',
        {},
        `${this.getFileNameDate('BP Assignment')}.xlsx`
      )
        .then((res) => {

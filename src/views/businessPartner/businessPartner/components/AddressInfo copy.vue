      ...
      >
        ...
        <template v-if="cdsQc === '1'">
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item :label="$t('bp.project')">
                <el-select
        ...
        <template v-if="cdsQc === '0'">
          ...
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item
                v-if="!termForm.businessAddressId"
                :label="`${$t('mapLang.addressCode')}`"
                :rules="[
                  {
                    required: termForm.isAutoAddressCode === '0',
                    pattern: new RegExp(/^(?!(\s+$))/g),
                    message: $t('ui.reqMsg').replace('$1', $t('mapLang.addressCode')),
                    trigger: ['blur', 'change']
                  }
                ]"
                prop="addressCode"
              ...
              <el-form-item :label="`${$t('mapLang.addressCode')}`" prop="addressCode" v-else>
                <el-input disabled v-model="termForm.addressCode" maxlength="50" />
              </el-form-item>
            </el-col>
          </el-row>
        </template>
        ...
        <template v-if="cdsQc === '1'">
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item :label="$t('mapLang.provinceState')" prop="provinceState">
                <el-select
            ...
            <el-col :span="24">
              <el-form-item :label="`${$t('ui.addressLine1')}`" prop="address1">
                <el-autocomplete
                  ...
                  :disabled="!(termForm.cityTown || termForm.provinceState)"
                  placeholder=""
                  @select="handleSelect"
                  @blur="handleAddressBlur"
                  :maxlength="200"
                ></el-autocomplete>
                <div v-show="false" id="searchId"></div>
              </el-form-item>
            </el-col>
...
export default {
        ...
        provinceState: [
          {
            ...
            // eslint-disable-next-line
            pattern: new RegExp(/^(?!(\s+$))/g),
            message: this.$t('ui.reqMsg').replace('$1', this.$t('mapLang.provinceState')),
            trigger: ['change']
          }
        ]
      },
  ...
  methods: {
    ...
    querySearchAsync(queryString, cb) {
      // const res = [{ value: 'shen zhen shi', id: 2332 }]
      if (!queryString) {
        const arr1 = []
        cb(arr1)
        return
      }
      if (queryString.length < this.searchMapAddressMiniMum) {
        const arr1 = []
        cb(arr1)
        return
      }

      const service1 = new window.google.maps.places.AutocompleteService()
      const input = []
      if (this.termForm.country) {
        input.push(this.termForm.country)
      }
      if (this.termForm.provinceState) {
        input.push(this.termForm.provinceState)
      }
      if (this.termForm.cityTown) {
        input.push(this.termForm.cityTown)
      }
      if (queryString) {
        input.push(queryString)
      }
      service1.getQueryPredictions({ input: input.join(','), language: 'en' }, (res, status) => {
    ...
    handleSelect(item) {
      this.$set(this.termForm, 'address1', item.description)
      this.$set(this.termForm, 'validAddress', item.description)
      const searchId = document.querySelector('#searchId')
      const service = new window.google.maps.places.PlacesService(searchId)
      const request = {
      ...
      service.getDetails(request, (res, status) => {
        if (status === 'OK') {
          ...
          let city = {}
          let postalCode = {}
          const addressArr = res.address_components.filter((item) => {
          ...
          const addressData = {
            ...
            point: res.geometry.location
          }
          this.$set(this.termForm, 'country', addressData.country)
          if (!this.termForm.provinceState) {
            this.$set(this.termForm, 'provinceState', addressData.province || addressData.city)
          }
          if (!this.termForm.cityTown) {
            this.$set(this.termForm, 'cityTown', addressData.city || addressData.province)
          }

          this.$set(this.termForm, 'postalCode', addressData.postalCode)
          this.$set(this.termForm, 'address', addressData.address)
          this.$set(this.termForm, 'address1', addressData.address)
          this.$set(this.termForm, 'validAddress', addressData.address)
          this.$set(
            this.termForm,
            'latLng',
            addressData.point.lat() + ',' + addressData.point.lng()
          )
        }
      })
    },
    async handleAddressBlur() {
      if (!this.termForm.address) {
        this.$set(this.termForm, 'latLng', '')
        this.$set(this.termForm, 'postalCode', '')
        return
      }
      if (this.termForm.address1 !== this.termForm.validAddress || !this.termForm.latLng) {
        ...
        if (this.termForm.address1) {
          input.push(this.termForm.address1)
        }
        const res = await this.getLatLngFromAddress(input.join(','))
        if (res.latLng) {
          this.$set(this.termForm, 'latLng', res.latLng)
          if (!this.termForm.provinceState) {
            this.$set(
              this.termForm,
              'provinceState',
              res.addressData.province || res.addressData.city
            )
          }
          if (!this.termForm.cityTown) {
            this.$set(this.termForm, 'cityTown', res.addressData.city || res.addressData.province)
          }
          this.$set(this.termForm, 'postalCode', res.addressData.postalCode)
          this.$set(this.termForm, 'validAddress', this.termForm.address)
        } else {
          this.$set(this.termForm, 'latLng', '')
          this.$set(this.termForm, 'postalCode', '')
        }
        ...
        this.geocoder.geocode({ address }, (results, status) => {
          if (status === window.google.maps.GeocoderStatus.OK) {
            ...
            if (res) {
              ...
              const latLng = addressData.point.lat() + ',' + addressData.point.lng()
              resolve({
                latLng: latLng,
                addressData
              })
    ...
    addressChange(row) {
      if (row.addressData) {
        this.$set(this.termForm, 'country', row.addressData.country)
        this.$set(this.termForm, 'countryEn', row.addressData.country)
        this.$set(this.termForm, 'provinceState', row.addressData.province || row.addressData.city)
        this.$set(this.termForm, 'cityTown', row.addressData.city || row.addressData.province)
        this.$set(this.termForm, 'postalCode', row.addressData.postalCode)
      }
      this.$set(this.termForm, 'address', row.address)
      this.$set(this.termForm, 'address1', row.address)
      this.$set(this.termForm, 'validAddress', row.address)
      this.$set(this.termForm, 'latLng', row.latLng)
    },
    /* 国家相关操作 */
    getCountriesList() {
      if (this.cdsQc === '0') {
    ...
    countryIdChange() {
      const list = this.countryList.filter((x) => x.en === this.termForm.countryEn)
      // console.log(list, '===')
      this.termForm.provinceState && this.$set(this.termForm, 'country', list[0].name)

      this.termForm.provinceState && this.$set(this.termForm, 'provinceState', '')
    ...
    handleResetTerm() {
      ...
      if (this.cdsQc === '0') {
        this.$set(
          this.termForm,
          'countryId',
          this.countryItem ? this.countryItem.countryId : undefined
        )
        this.$set(this.termForm, 'country', this.countryItem ? this.countryItem.country : undefined)
      }
      // this.$nextTick(() => {
              ...
              checkInRange(params)
                .then((res) => {
                  if (params.projectIdList && params.projectIdList.length > 0) {
                    params.projectIdList = (params.projectIdList || []).join(',') || ''
                  }
                  vm.tableList.push({
                    ...params
                  })

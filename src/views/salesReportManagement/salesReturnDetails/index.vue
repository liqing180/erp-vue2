  ...
  <div class="app-container">
    ...
    <el-row :gutter="10">
      <el-col :span="1.5">
        <el-button
          ...
          @click="openExport"
          v-hasPermi="['salesReportManagement:salesReturnDetails:export']"
    ...
    >
      <el-table-column
        ...
        align="center"
        v-if="checkPermi(['salesReportManagement:salesReturnDetails:export'])"
      >
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          ...
          <template v-else-if="item.prop === 'returnQtyForBasic'">
            <template>{{
              $numberStr(scope.row[item.prop], $getDecNum(scope.row.decimalNum))
            }}</template>
          </template>
...
} from '@/api/salesReportManagement/salesReturnsSummary'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      conditionForSalesGoodsReturnNoList: [],
  ...
  methods: {
    ...
    initQueryParams() {
      ...
      this.$set(this.queryParams, 'startDateReturnDate', startTimer)
      this.$set(this.queryParams, 'endDateReturnDate', end)
    ...
    onDictReady() {
      this.tableList.forEach((item) => {
        item.salesOrderDetailStatusStr = this.selectDictLabel(
          this.dict.type.s_sales_order_detail_status,
          item.salesOrderDetailStatus
        )
      })
      this.$$getColumnContentMaxWidth(this.columns, this.tableList)
    },
    ...
    querySearchList() {
      querySearchListForCustomerProductOrder({}).then((res) => {
        ...
        this.conditionForSalesGoodsReturnNoList = (conditionForSalesGoodsReturnNoList || []).map(
          (item) => {
            return {
              value: item,
              label: item
        ...
        this.searchData.forEach((x) => {
          ...
          } else if (x.name === 'businessPartnerIdForSearchList') {
    ...
    setFuzzyData() {
      this.searchData.forEach((item) => {
        if (item.name === 'selectAnInputName') {
          ...
          } else if (item.selectId === 'conditionForSalesGoodsReturnNo') {
    ...
    handleExport(data) {
      ...
      this.$importOrExportDownFile(
        '/sales/report/salesReturnAnalysis/exportForCustomerProductOrder',
        params,
        `${this.getFileNameDate('Sales Return Details')}.xlsx`
      )
        .then((res) => {

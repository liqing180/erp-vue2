...
import {
  ...
  queryImageForSalesPerson,
  querySearchListForSalesPerson
} from '@/api/salesReportManagement/salesPerformanceSummary'
import PieChart from '@/views/salesReportManagement/salesDepartmentPerformanceSummary/PieChart'
...
export default {
  ...
  methods: {
    ...
    getList(query) {
      ...
      this.queryImageForSalesPerson()
    },
    queryImageForSalesPerson() {
      const { endDateCreatedTime, startDateCreatedTime } = this.queryParams

      queryImageForSalesPerson({ endDateCreatedTime, startDateCreatedTime }).then((res) => {
    ...
    radioChange() {
      const { imageForSalesPersonList, totalAmountSo, totalAmountSq, totalCountSq, totalCountSo } =
        this.chartOptions

      let imageForSalesDepartmentList0, imageForSalesDepartmentList1, imageForSalesDepartmentList2
      if (this.radio === '0') {
        imageForSalesDepartmentList0 = (imageForSalesPersonList || [])
          .filter((x) => x.totalAmountSq && x.totalAmountSq !== 0)
          .map((x) => {
            return {
              customTotalAmount: totalAmountSq,
              value: x.totalAmountSq,
              name: x.salesPersonBy,
              ...x
            }
          })
        imageForSalesDepartmentList1 = (imageForSalesPersonList || [])
        ...
        // console.log(imageForSalesDepartmentList1)
        imageForSalesDepartmentList2 = (imageForSalesPersonList || [])
    ...
    handleExport(data) {
      ...
      this.$importOrExportDownFile(
        ...
        params,
        `${this.getFileNameDate('Sales Performance Summary')}.xlsx`
      )
        .then((res) => {

    ...
    <div class="app-container" v-if="currentIndex === 0">
      ...
      <el-row :gutter="10">
        <el-col :span="1.5">
          <el-button
            ...
            @click="openExport"
            v-hasPermi="['salesReportManagement:salesSummary:export']"
      ...
      >
        <el-table-column
          ...
          align="center"
          v-if="checkPermi(['salesReportManagement:salesSummary:export'])"
        >
        </el-table-column>
      ...
      <div class="flex mt20">
        <div style="width: 50%">
          <PieChart ref="PieChart" :title="$t('SALES.amountProportionAnalysisCustomer')" />
        </div>
        <div style="width: 50%">
          <PieChart ref="PieChart1" :title="$t('SALES.amountProportionAnalysisCountry')" />
        </div>
      </div>
    </div>
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          label: vm.$t('SALES.deliveredAmount'),
          minWidth: 160,
          visible: true,
          tooltip: true,
          padding: 50,
  ...
  methods: {
    ...
    querySearchList() {
      querySearchListForCustomer({}).then((res) => {
        const data = res.data || {}
    ...
    queryImageForCustomer() {
      ...
      queryImageForCustomer({ endDateCreatedTime, startDateCreatedTime }).then((res) => {
        const data = res.data || {}
        let salesCustomerAmountImageList = data.salesCustomerAmountImageList || []
        let salesCountryAmountImageList = data.salesCountryAmountImageList || []
        const totalAmountForCustomer = data.totalAmountForCustomer || ''
        const totalAmountForCountry = data.totalAmountForCountry || ''

        salesCustomerAmountImageList = salesCustomerAmountImageList.map((x) => {
          return {
            value: x.salesAmountForBasic,
            name:
              (x.reportName || x.businessPartnerName) +
              ' ' +
              (this.$numberStr(x.salesAmountForBasic, 2) || ''),
            topLabel: this.$t('SALES.customerName'),
            showName: x.reportName || x.businessPartnerName,
            totalAmount: totalAmountForCustomer
          }
        })
        salesCountryAmountImageList = salesCountryAmountImageList.map((x) => {
          return {
            value: x.salesAmountForBasic,
            name: x.country + ' ' + (this.$numberStr(x.salesAmountForBasic, 2) || ''),
            topLabel: this.$t('SALES.country'),
            showName: x.country,
            totalAmount: totalAmountForCountry
          }
        })
        this.$nextTick(() => {
          this.$refs.PieChart && this.$refs.PieChart.initChart(salesCustomerAmountImageList)
          this.$refs.PieChart1 && this.$refs.PieChart1.initChart(salesCountryAmountImageList)
        })
      })
    },
    ...
    handleExport(data) {
      ...
      this.$importOrExportDownFile(
        'sales/report/salesSummary/exportForCustomer',
        params,
        `${this.getFileNameDate('Sales Summary')}.xlsx`
      )
        .then((res) => {

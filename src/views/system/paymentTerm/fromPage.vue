<template>
  ...
  <FormPageLayout v-loading="submitLoading">
    <template v-slot:btn>
      ...
      <el-button type="primary" v-if="saveDraftShow" size="mini" @click="handleSaveDraft"
        >{{ $t('uiBtn.saveDraft') }}
      </el-button>
      ...
      <el-collapse v-model="activeNames">
          ...
          <el-collapse-item name="1">
            ...
            >
              ...
              <el-row>
                ...
                <el-col :span="8">
                  <el-form-item :label="`${$t('ui.paymentTermCode')}`" prop="paymentTermCode">
                    <el-input v-model="form.paymentTermCode" maxlength="50" />
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                  <el-form-item
                    :label="`${$t('ui.paymentTermNo')}`"
                    :rules="[
                      {
                        required: form.isAutoNo === '0',
                        pattern: new RegExp(/^(?!(\s+$))/g),
                        message: $t('ui.reqMsg').replace('$1', $t('ui.paymentTermNo')),
                        trigger: ['blur']
                      }
                    ]"
                    prop="paymentTermNo"
                  >
                    ...
                    <div class="input-switch-box">
                      <div class="con-left">
                        <el-input
                          :disabled="form.isAutoNo === '1' || !!rowId"
                          v-model="form.paymentTermNo"
                          maxlength="50"
                        />
                      </div>
                      <div class="con-right" v-if="!rowId">
                        <el-switch
                          v-model="form.isAutoNo"
                          active-value="1"
                          inactive-value="0"
                          @change="changeEmployeeNoSwitch('paymentTermNo')"
                        ></el-switch>
                      </div>
                    </div>
                  </el-form-item>
              ...
              <el-row>
                <el-col :span="8">
                  <el-form-item :label="`${$t('ui.purpose')}`" prop="paymentTermPurposeList">
                    <el-select
                      v-model="form.paymentTermPurposeList"
                      :title="
                        selectDictLabels(
                          dict.type.payment_term_purpose,
                          form.paymentTermPurpose || ''
                        )
                      "
                      :disabled="comDisFrom"
                      placeholder=""
                      ...
                      style="width: 100%"
                      clearable
                      @change="handlePaymentTermPurpose"
                    >
                      <el-option
                        v-for="dict in dict.type.payment_term_purpose"
                        :key="dict.value"
                        :label="dict.label"
                        :value="dict.value"
                      ></el-option>
                ...
                <el-col :span="8">
                  <el-form-item :label="`${$t('ui.status')}`">
                    <el-input :value="form.paymentTermStatusShowStr" disabled></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
        ...
        <div class="form-card mt10">
          <el-collapse-item name="2">
            ...
            <div>
              <div class="pb20">
                <el-row :gutter="10" class="mb8" v-if="!comDisStage">
                  <el-col :span="1.5">
                    <el-button
                      type="primary"
                      icon="el-icon-plus"
                ...
                >
                    ...
                    <template slot-scope="scope">
                      <template v-if="item.prop === 'stageName'">
                        ...
                        <template v-if="scope.row.isTotal !== '1'">
                          ...
                          <span
                            v-if="
                              (scope.row.childList || []).length <= 0 && scope.row.levelNum === 2
                            "
                        ...
                        <span :class="[scope.row.isTotal === '1' ? 'weight' : '']" v-else>{{
                          $t('system.total')
                        }}</span>
                      </template>
                      <el-tag
                      ...
                      <template v-else-if="item.prop === 'percentage'">
                          ...
                          <div class="flexSb">
                            <div class="flow1">
                              <el-input-number
                                ...
                                :min="1"
                                :max="
                                  scope.row.childList && scope.row.childList.length > 0
                                    ? 999999999
                                    : 100
                                "
                                :disabled="scope.row.childList && scope.row.childList.length > 0"
                            ...
                            >
                              <el-tooltip placement="top" effect="light">
                                <div slot="content">
                                  <div>{{ $t('system.automatically') }}</div>
                                </div>
                                <svg-icon
                                  icon-class="lock_2"
                                  class="ml5"
                        ...
                        <template v-if="scope.row.isTotal === '1'">
                          <div class="flexStart weight">
                            <div
                              :style="{
                                color:
                                  scope.row[item.prop] && scope.row[item.prop] === 100
                                    ? '#81bc5e'
                                    : '#F56C6C',
                                marginLeft: '15px'
                              }"
                            ...
                            <i
                              class="el-icon-warning ml20 mr10"
                              style="color: #9e9b96; font-size: 20px"
                            ></i>
                            <span style="color: #9e9b96">
                              {{ $t('system.amountMust100') }}
                            </span>
                          </div>
                        </template>
                  ...
                  <el-table-column
                    ...
                    fixed="right"
                  >
                    <template slot-scope="scope" v-if="scope.row.isTotal !== '1'">
                      <div class="flex" style="flex: 1">
                        <div class="flexCCen" style="flex: 0 0 35px">
                          <i
                        ...
                        <div class="flexCCen" style="flex: 0 0 35px" v-if="scope.row.levelNum < 2">
                          <i
                            class="el-icon-plus pointer"
                            style="font-size: 20px; color: #409eff"
                            :title="$t('uiBtn.add')"
                            @click="handleAddStage(scope.row)"
                          ></i>
                        </div>
                        <div
                          class="flexCCen"
                          style="flex: 0 0 35px"
                          v-if="rowId ? scope.$index !== 0 : true"
                        >
                          <i
    ...
    <addStageFormDlg ref="addStageFormDlg" @onSuccess="updateStage" :comDisFrom="comDisStage" />
  </FormPageLayout>
</template>

<script>
import { formDirtyClass } from '@/mixins/formDirtyClass'
import { submitPaymentTerm, savePaymentTerm, queryPaymentTermById } from '@/api/system/paymentTerm'
...
export default {
  ...
  mixins: [pageMixin],
  components: { SystemOperationLogTable, addStageFormDlg },
  data() {
    const vm = this
    return {
      saveKey: '1',
  ...
  computed: {
    comTableList() {
      ...
      table.forEach((row) => {
        if (row.childList && row.childList.length > 0) {
          let fatherPercentage = 0
          row.childList.forEach((child) => {
            if (this.$resultOfBoolean(child.percentage)) {
              fatherPercentage += child.percentage * 1
            }
          })
          row.percentage = fatherPercentage
        }
        if (this.$resultOfBoolean(row.percentage)) {
          percentage += row.percentage * 1
        }
      })
      percentage = table.some((x) => this.$resultOfBoolean(x.percentage)) ? percentage : undefined
    ...
    saveDraftShow() {
      if (this.rowId) {
        return this.editAuth && this.form.paymentTermStatus === '1'
      }
      return true
    },
    comDisStage() {
  ...
  methods: {
    ...
    handleUpdate() {
      ...
      queryPaymentTermById({ paymentTermId: rowId }).then((response) => {
        ...
        if (valid) {
          ...
          this.$modal
            ...
            .then(() => {
              ...
              return submitPaymentTerm(param)
            })
            .then((response) => {
    ...
    getFormJson() {
      ...
      param.paymentTermDetailList = this.tableList
    ...
    handleSaveDraft(isConfirm = true, gotoRoute) {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (isConfirm) {
            this.$modal
              .confirm(this.$t('ui.savePTConfirm'))
              .then(() => {
                this.submitLoading = true
              ...
              .then((response) => {
                ...
                this.$modal.msgSuccess(
                  this.$t('ui.savePTSuccess').replace('$1', param.paymentTermName)
                )
                setTimeout(() => {
                  this.submitLoading = false
                }, 500)

<template>
  ...
  >
    <el-form ref="formRef1" class="mr40" :model="form" :rules="rules" label-width="180px">
      <el-row>
        <el-col :span="12">
          <el-form-item prop="deliveryProcedure">
            <span slot="label">
              <el-tooltip placement="top">
                <div slot="content" v-html="$t('Operation.deliveryProcedureTip')"></div>
                <i class="el-icon-question"></i>
              </el-tooltip>
              {{ $t('Operation.deliveryProcedure') }}
            </span>
            <el-select class="w100" v-model="form.deliveryProcedure">
              <el-option
                v-for="item in dict.type.dm_delivery_procedure_type"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              >
    ...
    return {
      ...
      rules: {
        deliveryProcedure: [
          {
            required: true,
            message: this.$t('ui.reqMsg').replace('$1', this.$t('Operation.deliveryProcedure')),
            trigger: ['blur', 'change']
          }
        ]
      }
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      querySetupFieldById(id).then((response) => {
        if (response.code === 200) {
          ...
          this.form = data
          this.$set(this.form, 'deliveryProcedure', config.DELIVERY_PROCEDURE)
          this.open = true
        }
      })
    },
    ...
    submitForm: function () {
      ...
      this.$refs.formRef1.validate((valid) => {
        ...
        if (valid) {
          this.form.customConfigFields.DELIVERY_PROCEDURE = this.form.deliveryProcedure

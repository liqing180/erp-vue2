<template>
  ...
  >
    <el-form ref="formRef1" class="mr40" :model="form" :rules="rules" label-width="250px">
      <el-row>
        <el-col :span="12">
          <el-form-item :label="$t('Operation.timeOffline')" prop="timeOffline">
            <div style="display: flex">
              <el-input-number
                style="width: 100%"
                v-model="form.timeOffline"
                ...
                placeholder="1 - 999"
                :disabled="comDisFrom"
                @change="inputNumberChange('timeOffline')"
              />
              <div class="unit">
                {{ form.customConfigFields && form.customConfigFields.ROUTE_TRACKING_UNIT }}
              </div>
            </div>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="$t('Operation.moduleItBelongsTo')">
            <el-input disabled :value="$t('Operation.routeTracking')" maxlength="50" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        ...
        timeOffline: [
          {
            ...
            message: this.$t('ui.reqMsg').replace('$1', this.$t('Operation.timeOffline')),
            trigger: ['blur']
          }
        ]
      }
    ...
    handleUpdate(row) {
      ...
      querySetupFieldById(id).then((response) => {
        if (response.code === 200) {
          ...
          this.form = data
          this.$set(this.form, 'timeOffline', config.ROUTE_TRACKING_TIME_OFFLINE)
          this.open = true
        }
      })
    },
    ...
    submitForm: function () {
      ...
      this.$refs.formRef1.validate((valid) => {
        ...
        if (valid) {
          ...
          this.form.customConfigFields.ROUTE_TRACKING_TIME_OFFLINE = this.form.timeOffline

<template>
  ...
  >
    <el-form ref="formRef1" class="mr40" :model="form" :rules="rules" label-width="180px">
      <el-row>
        <el-col :span="12">
          <el-form-item :label="$t('Operation.application')" prop="application">
            <el-select
              style="width: 100%"
        ...
        application: [
          {
            ...
            message: this.$t('ui.reqMsg').replace('$1', this.$t('Operation.application')),
            trigger: ['change', 'blur']
          }
        ]
      }
    ...
    handleUpdate(row) {
      ...
      querySetupFieldById(id).then((response) => {
        if (response.code === 200) {
          ...
          this.form = data
          this.$set(this.form, 'application', config.APPLICATION || [])
          this.open = true
        }
      })
    },
    ...
    submitForm: function () {
      ...
      this.$refs.formRef1.validate((valid) => {
        ...
        if (valid) {
          ...
          this.form.customConfigFields.APPLICATION = this.form.application

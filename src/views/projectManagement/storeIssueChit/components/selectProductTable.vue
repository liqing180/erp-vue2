<template>
  <div>
    <div
      ...
      :style="{
        left: tooltipLeft !== undefined ? `${tooltipLeft}px` : undefined,
        right: tooltipRight !== undefined ? `${tooltipRight}px` : undefined,
        top: `${tooltipY}px`,
        zIndex: 9999
      }"
    >
    ...
    >
        ...
        <el-tabs v-model="activeWarehouseId" type="card" @tab-click="handleTabClick">
          ...
          >
            <span slot="label">
              ...
              <span class="ml5">
                <span v-if="selectList.filter((p) => p.warehouseId === item.warehouseId).length"
                  >({{ selectList.filter((p) => p.warehouseId === item.warehouseId).length }})</span
                >
              </span>
            </span>
          </el-tab-pane>
      ...
      <div style="display: flex; align-items: flex-start" v-loading="loading">
          ...
          <pagination
            :selectedNum="
              $$getFilteredSelectedNum(
                ...
                tableList,
                selectList.filter((item) => item.warehouseId === activeWarehouseId),
                rowIdKey
              )
            "
            :saveKey="saveKey"
...
import { querySICCanSelectProductList } from '@/api/projectManagement/storeIssueChit'
export default {
  directives: {
    // transferDom
  },
  ...
  data() {
    ...
    return {
      ...
      filteredTreeData: [],
      showTooltip: false,
      tooltipText: vm.$t('PRODUCT.productAvailableTip'),
      tooltipLeft: 0,
      tooltipRight: undefined,
      tooltipY: 0,
      currentRow: null, // 标记当前悬浮的行
  ...
  methods: {
    handleTabClick() {
      ...
      this.$nextTick(() => {
        let alreadySelectIdList = this.selectList
    ...
    handleGlobalMouseMove(event) {
      if (this.showTooltip && this.currentRow) {
        // 偏移10px避免遮挡鼠标
        this.tooltipLeft = event.clientX + 14
        ...
        // 右边界：如果超出视口，鼠标左侧显示
        if (this.tooltipLeft + tooltipWidth > viewportWidth) {
          this.tooltipLeft = undefined
          this.tooltipRight = 30
        }
      }
    },
    handleCellMouseEnter(row) {
      if (!row.maxQty || row.maxQty <= 0) {
        this.currentRow = row
    ...
    setActiveWarehouseId() {
      if (this.activeWarehouseId) {
        if (!this.warehouseList.find((item) => item.warehouseId === this.activeWarehouseId)) {
          this.activeWarehouseId = this.warehouseList[0].warehouseId
        }
      } else {
        this.activeWarehouseId = this.warehouseList[0].warehouseId
      }
    ...
    handleAdd(selectList, alreadyIdList) {
      /* 借货单单独更新产品列表，已存在的产品直接过滤掉 */
      this.alreadyIdList = alreadyIdList || []
      document.addEventListener('mousemove', this.handleGlobalMouseMove)
      this.selectList = selectList || []
      this.setActiveWarehouseId()
      // this.queryParams.productCategoryId = ''
    ...
    getList() {
      ...
      if (this.alreadyIdList && this.alreadyIdList.length) {
        param.alreadyProductIdList.push(...this.alreadyIdList)
      }
      param.warehouseId = this.activeWarehouseId
      param.storeIssueChitId = this.storeIssueChitId
      if (this.costProjectId) {
        param.costProjectId = this.costProjectId
      }
      param.selectedIdList = this.$$getSelectedIdList(
        this.selectList.filter((item) => item.warehouseId === this.activeWarehouseId),
        this.rowIdKey
      )
      this.tableList = []

      this.loading = true
      querySICCanSelectProductList(param)
        .then((response) => {
          this.loading = false
    ...
    selectable(row, rowIndex) {
      if (!row.maxQty || row.maxQty <= 0) return false
      return true // 不禁用
    },
    tableRowClassName({ row, rowIndex }) {
      let color = 'pointer'
      if (!row.maxQty || row.maxQty <= 0) {
        color = 'table-disSelected-bgcolor'
      }
      return color
    },
      ...
      if (selection.length) {
        ...
        selection.forEach((item) => {
          if (!curSelectedIds.includes(item[this.rowIdKey])) {
            item.warehouseId = this.activeWarehouseId
    ...
    handleSelectionChange(selection, row) {
      ...
      row.warehouseId = this.activeWarehouseId
    ...
    handleRowClick(row, column) {
      ...
      if (!row.maxQty || row.maxQty <= 0) return

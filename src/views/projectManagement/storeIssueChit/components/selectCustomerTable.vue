...
import {
  ...
  queryCanSelectCustomerSearchList
} from '@/api/salesManagement/salesInquiry'

import locale from '@/views/organization/lang/index'

export default {
  dicts: ['bp_business_partner_status', 'bp_customer_type', 'bp_customer_group'],
  mixins: [pageMixin],
  props: {
    warehouseId: {
      type: String,
  ...
  data() {
    ...
    return {
      ...
      searchData: [
        ...
        //   width: '200px',
        //   selectValue: 'customerGroup',
        //   selectLabel: 'customerGroupShowStr',
        //   selectData: []
        // }
      ]
    }
  ...
  methods: {
    handleOpen() {
      ...
      this.visible = true
      this.getList()
      this.queryCanSelectCustomerSearchList()
    },
    queryCanSelectCustomerSearchList() {
      queryCanSelectCustomerSearchList({ menuPerms: this.menuKey.storeIssueChit }).then((res) => {
        const data = res.data || {}
        const customerGroupList = data.customerGroupList || []
        this.searchData.forEach((item) => {
          if (item.name === 'customerGroupList') {
            item.selectData = customerGroupList
          }
        })
      })
    },

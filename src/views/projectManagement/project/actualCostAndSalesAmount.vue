<template>
  <el-collapse v-model="activeNames">
    <div class="form-card mt10" v-if="type !== 'salesAmount'">
      <el-collapse-item name="1">
        <FormCollapseItemTitle
          slot="title"
          :title="
            type === 'actualCost'
              ? form.costProjectCode + ' ' + $t('PRODUCT.actualCost')
              : $t('PRODUCT.actualCost')
          "
        >
        </FormCollapseItemTitle>
        <!-- 一级 -->
    ...
    <div
      :class="['form-card mt10', type === 'salesAmount' ? '' : 'mt10']"
    ...
    >
      <el-collapse-item name="2">
        <FormCollapseItemTitle
          ...
          :title="
            type === 'salesAmount'
              ? form.costProjectCode + ' ' + $t('PRODUCT.salesAmount')
              : $t('PRODUCT.salesAmount')
          "
        >
        </FormCollapseItemTitle>
        <!-- 一级 -->
...
export default {
  ...
  methods: {
    init(row) {
      this.form = row || {}
      this.rowId = row.costProjectId
      this.costProjectName = row.costProjectName
      this.type = undefined
      this.activeName = 1
      this.$nextTick(() => {

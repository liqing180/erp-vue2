...
import { queryCanSelectCostProjectListForPage } from '@/api/projectManagement/projectTransfer'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      const alreadyId =
        this.type === 'transferFrom'
          ? this.form.transferToCostProjectId
          : this.form.transferFromCostProjectId
      param.alreadyId = alreadyId
      this.loading = true
          ...
          this.$nextTick(() => {
            ...
            if (alreadyId1) {
              this.tableList.forEach((item) => {
                if (alreadyId1 === item[this.rowIdKey]) {
    ...
    submit() {
      ...
      this.$emit('onSuccess', this.selectList[0], this.type)
      this.visible = false
    }
  }
}

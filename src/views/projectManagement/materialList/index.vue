<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          @click="openExport"
          v-hasPermi="['projectManagement:materialList:export']"
    ...
    >
      <el-table-column
        v-if="checkPermi(['projectManagement:materialList:export'])"
        type="selection"
        width="50"
        align="center"
...
<script>
import {
  queryMaterialListList,
  querySearchList,
  queryMaterialListOperationLog
...
export default {
  ...
  data() {
    ...
    return {
      ...
      searchData: [
        ...
        {
          name: 'materialListNo',
          label: vm.$t('PRODUCT.materialListNo'),
          type: 'InputEle',
          isLine: false
        },
        {
          name: 'materialListStatusList',
  ...
  methods: {
    ...
    handleOperationLog(row) {
      this.$refs.systemOperationLogTableDlg.handleOpen(queryMaterialListOperationLog, {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.materialListId)
    ...
    handleUpdate(row, column) {
      ...
      if (this.editAuth) {
        this.$router.push({
          path: '/projectManagement/editMaterialList',
          query: {
            id: row.materialListId,
            timeId: Date.now()
          }
        })
        return
      ...
      this.$router.push({
        path: '/projectManagement/viewMaterialList',
        query: {
          id: row.materialListId,
          timeId: Date.now()
        }
    ...
    handleExport(data) {
      ...
      if (exportType === 2) {
        params.exportIdList = vm.tableList.map((item) => item.materialListId)
      }
      if (exportType === 3) {
      ...
      this.$importOrExportDownFile(
        '/sales/materialList/export',
        params,
        `${this.getFileNameDate('Material List')}.xlsx`
      )
        .then((res) => {

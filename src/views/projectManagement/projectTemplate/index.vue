  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="el-icon-plus"
          size="mini"
          v-hasPermi="['projectManagement:projectTemplate:add']"
          @click="handleAdd"
      ...
      <el-col :span="1.5">
        <el-dropdown
          ...
          v-hasPermi="['projectManagement:projectTemplate:edit']"
          v-if="!multiple"
          trigger="click"
        >
    ...
    >
      <el-table-column
        ...
        align="center"
        v-if="checkPermi(['projectManagement:projectTemplate:edit'])"
      />
...
export default {
  ...
  data() {
    ...
    return {
      ...
      columns: [
        ...
        {
          prop: 'projectTemplateStatus',
          propBy: 'projectTemplateStatusStr',
      ...
      searchData: [
        ...
        {
          name: 'projectTemplateStatusList',
          label: vm.$t('ui.status'),
          type: 'MultipleSelectEle',
          width: '200px',
          selectValue: 'dictValue',
          selectLabel: 'dictLabel',
          dict: 'project_template_status',
          selectData: []
        },
        {
          name: 'isActiveList',
  ...
  methods: {
    getList() {
      ...
      this.loading = true
      queryProjectTemplateList(param)
        .then((response) => {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.projectTemplateId)
    ...
    handleChangeUsersStatus(type) {
      const names = this.selectList.map((item) => item.projectName).join(', ')
      const isActive = type === 'active' ? '1' : '0'
      let confirmMsg = this.$t('PRODUCT.changeProjectInactive').replace('$1', names)
      if (type === 'active') {
        confirmMsg = this.$t('PRODUCT.changeProjectActive').replace('$1', names)
      }
      this.$modal
        .confirm(confirmMsg)
        .then(() => {
          return batchUpdateIsActive({ projectTemplateIdList: this.ids, isActive })
        })
        .then(() => {
          this.getList()
        })
    ...
    handleAdd(row) {
      this.$router.push({
        path: '/projectManagement/addProjectTemplate',
    ...
    handleUpdate(row, column) {
      ...
      this.$router.push({
        path: '/projectManagement/editProjectTemplate',
        query: { id: row.projectTemplateId, timeId: +new Date() }
      })
    },
    /** 搜索 */
    handleSearchForm() {
    ...
    handleExport(data) {
      ...
      if (exportType === 2) {
        params.exportIdList = vm.tableList.map((item) => item.projectTemplateId)
      }
      if (exportType === 3) {

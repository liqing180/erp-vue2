<template>
  <el-dialog
    v-dialogDrag
    v-dialogDragWidth
    :title="$t('PRODUCT.combinationProductList')"
...
import { queryCanSelectBomList } from '@/api/projectManagement/projectTemplate'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      searchData: [
        {
          name: 'condition',
          placeholder: `${this.$t('PRODUCT.combinationProductNo')} / ${this.$t(
            'PRODUCT.combinationProductName'
          )} / ${this.$t('PRODUCT.alias')}`,
          type: 'InputEle'
        }
      ],
      treeList: [],
  ...
  methods: {
    ...
    handleRowClick(row, column) {
      ...
      // this.visible = false
      const index = this.selectList.findIndex((item) => item.bomId === row.bomId)
      ...
      this.ids = this.selectList.map((item) => item.bomId)
    ...
    close() {
      ...
      this.$refs.tables && this.$refs.tree.setCheckedKeys([])
    },
    addAll() {

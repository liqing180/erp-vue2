...
export default {
  ...
  methods: {
    ...
    getList() {
      const vm = this
      const { productCategoryId, ...param } = this.queryParams
      param.productType = '2'
      param.alreadyProductIdList = this.alreadySelectIdList
      param.targetCurrencyId = this.currencyId
      ...
      queryCanSelectServiceList(param)
        .then((response) => {
          ...
          rows.forEach((item) => {
            item.valuationAmountStr = this.$numberStr(item.valuationAmount, 2)

    ...
    <el-collapse v-model="activeNames" v-show="showDetailList || showCommonFileList" class="mt20">
      <div class="form-card">
        ...
        <el-collapse-item name="3" v-if="beforeProductList.length > 0">
          <template slot="title">
            <strong>{{ $t('menu.projectTask') }} ({{ $t('uiBtn.beforeModification') }})</strong>
          </template>
          <div class="mb20">
            <el-table
        ...
        <el-collapse-item name="4" v-if="afterProductList.length > 0">
          <template slot="title">
            <strong>{{ $t('menu.projectTask') }} ({{ $t('uiBtn.afterModification') }})</strong>
          </template>
          <div class="mb20">
            <el-table
...
import { queryUpdateMsgProjectTask } from '@/api/projectManagement/projectTask'

export default {
  mixins: [createRevisionDisplayMixin({ switches: [], dictionaries: { serviceType: 'service_type', taskType: 's_task_type' } })],
  ...
  methods: {
    ...
    async handleOpen(id) {
      ...
      queryUpdateMsgProjectTask({ projectTaskId: id }).then((res) => {

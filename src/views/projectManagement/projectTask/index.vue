<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          @click="openExport"
          v-hasPermi="['projectManagement:projectTask:export']"
    ...
    >
      <el-table-column
        v-if="checkPermi(['projectManagement:projectTask:export'])"
        type="selection"
        width="50"
        align="center"
...
export default {
  name: 'ProjectTask',
  dicts: ['approved_status', 'project_task_status'],
  mixins: [pageMixin],
  ...
  data() {
    ...
    return {
      ...
      searchData: [
        ...
        {
          name: 'projectTaskNo',
          label: vm.$t('PRODUCT.projectTaskNo'),
          type: 'InputEle',
          isLine: false
        },
        {
          name: 'projectTaskStatusList',
          label: vm.$t('ui.status'),
          type: 'MultipleSelectEle',
          width: '200px',
          selectValue: 'dictValue',
          selectLabel: 'dictLabel',
          dict: 'project_task_status',
          selectData: []
        },
        {
  ...
  methods: {
        ...
        .then((response) => {
          ...
          rows.forEach((item) => {
            ...
            item.projectTaskStatusStr = this.selectDictLabel(
              this.dict.type.project_task_status,
    ...
    handleOperationLog(row) {
      this.$refs.systemOperationLogTableDlg.handleOpen(queryProjectTaskOperationLog, {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.projectTaskId)
    ...
    handleUpdate(row, column) {
      ...
      if (this.editAuth) {
        this.$router.push({
          path: '/projectManagement/editProjectTask',
          query: {
      ...
      this.$router.push({
        path: '/projectManagement/viewProjectTask',
        query: {
          id: row.projectTaskId,
          timeId: Date.now()
        }
    ...
    handleExport(data) {
      ...
      if (exportType === 2) {
        params.exportIdList = vm.tableList.map((item) => item.projectTaskId)
      }
      if (exportType === 3) {
      ...
      this.$importOrExportDownFile(
        '/sales/projectTask/export',
        params,
        `${this.getFileNameDate('Project Task')}.xlsx`
      )
        .then((res) => {

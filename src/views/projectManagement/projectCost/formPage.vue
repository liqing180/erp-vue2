<template>
  <FormPageLayoutTabs>
    <template v-slot:btn>
      ...
      <div v-if="comeFrom === '2'" class="bpm-page-menu-title">{{ $t('menu.projectCost') }}</div>
    </template>
    <template v-slot:tabs>
      <el-tabs v-model="activeName" size="mini" type="card">
        <el-tab-pane :label="$t('menu.customerQuestionnaire')" name="0"></el-tab-pane>
        <el-tab-pane :label="$t('menu.projectCost')" name="1"></el-tab-pane>
      </el-tabs>
    </template>
          ...
          <div class="form-card">
            <el-collapse-item name="1">
              ...
              <el-form ref="form" :model="form" @submit.native.prevent label-width="160px" disabled>
                <el-row>
                  <el-col :span="8">
                    <el-form-item :label="`${$t('PRODUCT.projectCostNo')}`" prop="projectCostNo">
                      <el-input v-model="form.projectCostNo" :title="form.projectCostNo" />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item :label="`${$t('ui.status')}`" prop="status">
                      <el-input
                        :value="
                          selectDictLabel(dict.type.project_cost_status, form.projectCostStatus)
                        "
                        :title="
                          selectDictLabel(dict.type.project_cost_status, form.projectCostStatus)
                        "
          ...
          <div class="form-card mt10">
            <el-collapse-item name="3">
              <FormCollapseItemTitle
                ...
                :warning="collapseWarningProjectCost"
              >
              </FormCollapseItemTitle>
              <div class="pb20">
                <projectCost
...
<script>
import {
  queryProjectCostById,
  saveDraftProjectCost,
  saveProjectCost,
  approvedProjectCost,
  rejectedProjectCost,
  withdrawApproved
...
export default {
  ...
  data() {
    return {
      ...
      activeName: '1',
      aplVisible: false,
      approvedUrl: approvedProjectCost,
      rjVisible: false,
      rejectedUrl: rejectedProjectCost,
      buttonAuthMsg: {
        isCanApproved: '0',
        isCanRejected: '0',
        isCanRevise: '0',
      ...
      collapseWarningProjectCost: false,
  ...
  computed: {
    ...
    withdrawApproveBtnShow() {
      if (!this.checkPermi(['projectManagement:projectCost:withdrawApprove'])) {
        return false
      }
      let show = false

      if (this.form.projectCostId && this.buttonAuthMsg.isCanWithdrawApproved === '1') {
        show = true
      }
      return show
    },
    ...
    isView() {
      ...
      if (this.form.projectCostStatus !== '1') {
        return true
      }
      return false
    }
  ...
  methods: {
      ...
      this.$modal
        ...
        .then(() => {
          return withdrawApproved({
            businessId: this.form.projectCostId,
    ...
    handleUpdate() {
      this.reset()
      queryProjectCostById({ projectCostId: this.rowId }).then((res) => {
        const {
          buttonAuthMsg,
        ...
        this.$nextTick(() => {
          ...
          this.$refs.projectCost && this.$refs.projectCost.init(projectCostDetailList)
        })
    ...
    reset() {
      ...
      this.$refs.projectCost && this.$refs.projectCost.reset()
    },
    handleSaveDraft() {
      const tableList = this.$refs.projectCost.tableList
      if (tableList.length <= 0) {
        this.collapseWarningProjectCost = true
        this.$modal.msgError(
          this.$t('ui.fromIncomplete').replace('$1', this.$t('menu.projectCost'))
        )
        return
      }
      this.collapseWarningProjectCost = false
      const params = {
        projectCostId: this.rowId,
        modifiedTime: this.form.modifiedTime
      }
      params.projectCostDetailList = tableList.filter((x) => x.picUserId)
      this.btnDisabled = true

      saveDraftProjectCost(params)
        .then((res) => {
          this.back()
          setTimeout(() => {
    ...
    submitForm() {
      ...
      this.$modal
        .confirm(this.$t('PRODUCT.projectConfirm'))
        .then(() => {
          const params = {
            projectCostId: this.rowId,
            modifiedTime: this.form.modifiedTime
          ...
          params.projectCostDetailList = tableList.filter((x) => x.picUserId)
          return saveProjectCost(params)
        })
        .then((response) => {
          this.$modal.msgSuccess(
            this.$t('PRODUCT.projectSuccess').replace('$1', this.form.projectCostNo)
          )
          this.back()
        })
        .catch(() => {})
    ...
    aplSubmitSuccess() {
      const vm = this
      vm.$message.success(
        `${vm.$t('PRODUCT.projectCostApprovedSuccess')}`.replace('$1', vm.form.projectCostNo)
      )
    ...
    rjSubmitSuccess() {
      const vm = this
      vm.$message.success(
        `${vm.$t('PRODUCT.projectCostRejectedSuccess')}`.replace('$1', vm.form.projectCostNo)
      )

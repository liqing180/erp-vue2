            ...
            <div class="pb20">
              <el-row :gutter="10" class="mb8">
                <el-col :span="1.5">
                  <el-button
                    @click="handleAdd"
                    v-if="checkPermi(['productManagement:productConfigurationTable:add'])"
                ...
                >
                  ...
                  <template slot-scope="scope">
                    <template v-if="item.prop === 'productName'">
                      <span v-if="!!scope.row.id">{{ scope.row.productName }}</span>
                      <SelectInput
                        v-else
                        style="width: 98%"
                    ...
                    <template v-else-if="item.prop === 'salesUom'">
                      <span v-if="!!scope.row.id" :title="showUomLabel(scope.row[item.prop])">{{
                        scope.row[item.prop]
                      }}</span>
                      <el-select
                        style="width: 98%"
                    ...
                    <template v-else-if="item.prop === 'uomCoefficient'">
                      <div style="display: flex; width: 98%">
                        <el-input-number
                          ...
                          :min="1"
                          :max="99999"
                          :disabled="!!scope.row.id || scope.row.salesUom === scope.row.uom"
                        />
                        <span
                          class="unit flow1"
...
import {
  saveSales,
  updateSales,
  batchQueryProductExpansionSales
  ...
  methods: {
    ...
    handleUpdate() {
      ...
      batchQueryProductExpansionSales({
        ...
        const data = res.data || {}
        this.form = res.data
        const list = data.childProductExpansionSalesList || []
        list.forEach((item) => {

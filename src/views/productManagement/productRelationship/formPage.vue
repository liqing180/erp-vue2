<template>
  ...
  <FormPageLayout>
          ...
          <el-collapse-item name="1">
            ...
            <div class="pb20">
              <el-row :gutter="10" class="mb8">
                <el-col :span="1.5">
                  <el-button
                    v-if="checkPermi(['productManagement:productRelationship:add'])"
                    @click="handleAdd"
                    type="primary"
                    icon="el-icon-plus"
                    size="mini"
              ...
              >
                ...
                >
                  <template slot="header" slot-scope="{ column }">
                    <span
                      v-if="
                        ['supplierName', 'productName', 'purchaseUom', 'uomCoefficient'].indexOf(
                          item.prop
                        ) !== -1
                      "
                    >
                  ...
                  <template slot-scope="scope">
                    <template v-if="item.prop === 'supplierName'">
                      <span v-if="!!scope.row.id">{{ scope.row.businessPartnerName }}</span>
                      <el-select
                        v-else
                        style="width: 98%"
                        v-model="scope.row.businessPartnerId"
                        :title="scope.row.businessPartnerName"
                        filterable
                        placeholder=""
                        @change="supplierChange($event, scope.$index)"
                      >
                        <el-option
                    ...
                    <template v-else-if="item.prop === 'productName'">
                      <div class="flow1" v-if="!!scope.row.id">{{ scope.row.productName }}</div>
                      <SelectInput
                        v-else
                        style="width: 98%"
                        ...
                        clearable
                        :disabled="comDisFrom"
                        @click="openProductNameTable(scope.$index, scope.row.ivtProducts)"
                      />
                    </template>
                    <template v-else-if="item.prop === 'label'">
                      <div v-if="!!scope.row.id && scope.row.disabledLabel === '1'">
                        {{ scope.row.label }}
                      </div>
                      <SelectInput
                        v-else
                    ...
                    <template v-else-if="item.prop === 'purchaseUom'">
                      <span
                        v-if="!!scope.row.id && scope.row.disabledPurchaseUom === '1'"
                      ...
                      <el-select
                        ...
                        v-else
                        :disabled="
                          !scope.row.businessPartnerId ||
                          !scope.row.productId ||
                          scope.row.isPhantom === '1'
                        "
                        filterable
                      >
                        <el-option
                    ...
                    <template v-else-if="item.prop === 'uomCoefficient'">
                      <div style="display: flex; width: 98%">
                        <el-input-number
                          ...
                          :max="99999.999"
                          :disabled="
                            (!!!!scope.row.id && scope.row.disabledUomCoefficient === '1') ||
    ...
    <selectProductNameTable ref="selectProductNameTable" @onSuccess="onSuccess" />
...
import selectProductNameTable from '@/views/productManagement/productRelationship/selectProductNameTable'
import payToAddressDlg from './payToAddressDlg.vue'

export default {
  dicts: ['sys_user_sex'],
  mixins: [pageMixin],
  components: { selectProductNameTable, payToAddressDlg },
  data() {
    const vm = this
    return {
      id: '',
  ...
  methods: {
    ...
    // 供应商
    querySupplierList() {
      querySupplierList({ menuPerms: this.menuKey.productRelationship }).then((res) => {
        this.supplierList = res.data || []
      })
    },
    supplierChange(e, index) {
      this.rowIndex = index
      const arr = this.supplierList.filter((x) => x.businessPartnerId === e)
      this.$set(this.tableList[index], 'businessPartnerNo', arr[0].businessPartnerNo)
      this.$set(this.tableList[index], 'businessPartnerName', arr[0].businessPartnerName)
      this.getProductSheetList(this.tableList[index])
    ...
    onSuccess(e) {
      ...
      const { inventoryUom, internalPartNo, productId, includeDecimal, decimalNum, isPhantom } =
        ivtProducts[0]
      this.$set(this.tableList[this.rowIndex], 'inventoryUom', inventoryUom)
      if (isPhantom === '1') {
        this.$set(this.tableList[this.rowIndex], 'purchaseUom', inventoryUom)
        this.$set(this.tableList[this.rowIndex], 'uomCoefficient', 1)
      } else {
        this.$set(this.tableList[this.rowIndex], 'purchaseUom', undefined)
      }
      this.$set(this.tableList[this.rowIndex], 'isPhantom', isPhantom)

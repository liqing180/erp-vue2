...
export default {
  ...
  computed: {
    ...
    comSelectedNum() {
      if (this.activeName === '1') {
        return this.selectList1.filter((i) => !i.purchaseQuotationDetailId).length
      } else {
        return this.selectList2.filter((i) => !i.purchaseQuotationDetailId).length
      }
    }
  },
  watch: {},
  ...
  methods: {
    ...
    handleOpen(data = [], alreadyProductIdList) {

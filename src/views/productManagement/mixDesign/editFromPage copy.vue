    ...
    <template v-slot:content>
                ...
                <el-row>
                  ...
                  <el-col :span="8">
                    <el-form-item
                      ...
                      :class="[isModified('scheduledTransferDate')]"
                    >
                      <el-date-picker
            ...
            <el-collapse-item name="3">
              ...
              >
                <span class="info-item mr20">
                  {{ $t('INVENTORY.product') }} : {{ form.variationList.length }}
                </span>
              </FormCollapseItemTitle>
              <div>
                <ProductInfo
      ...
      <div v-show="activeName === 'stockOutInDetails'">
        <el-collapse v-model="activeNames">
          ...
          <div class="form-card mt10" v-if="form.deliveryNoticeShipping">
...
export default {
  ...
  computed: {
    ...
    cancelBtnShow() {
      if (!this.checkPermi(['productManagement:mixDesign:cancel'])) {
        return false
      }
      let show = false
      if (
    ...
    withdrawApproveBtnShow() {
      if (!this.checkPermi(['productManagement:mixDesign:withdrawApprove'])) {
        return false
      }
      let show = false
  ...
  methods: {
      ...
      queryTransferOrderById({
        ...
        setTimeout(() => {
          ...
          this.$refs.uploadRef6 &&
            this.$refs.uploadRef6.initFileList(data.deliveryNoticeShipping.commonFileList || [])
        }, 300)
      })
    },
    // 表单重置
    ...
    updateWarehouse(row, type) {
      if (type === 'to') {
        ...
        this.$set(this.form, 'toUserBy', row.userName)

<template>
  <transition name="el-zoom-in-top" @after-enter="handleEnter" @after-leave="handleLeave">
    <div
      v-show="visible"
      class="el-picker-panel el-date-picker el-popper my-date-picker"
      :class="[
        {
      ...
      <div class="flex">
        <div class="el-picker-panel__body-wrapper">
          <div class="el-picker-panel__body">
            <div class="el-date-picker__time-header" v-if="false">
        ...
        <div class="time-wrapper" style="width: 120px">
          <TimePicker
            ref="TimePicker"
            :curValue="value ? value.getTime() : null"
            :defaultTime="defaultTime"
            @pick="handleTimePick2"
          />
        </div>
      </div>
...
export default {
  ...
  watch: {
    ...
    visible(val) {
      if (val) this.$nextTick(() => this.$refs.TimePicker.showDefault())
    },
  ...
  methods: {
    ...
    handleTimePick2(value) {
      if (isDate(value)) {
        let newDate
        if (this.value) {
          newDate = modifyTime(this.value, value.getHours(), value.getMinutes(), value.getSeconds())
        } else {
          newDate = modifyWithTimeString(this.getDefaultValue(), this.defaultTime)
          newDate = modifyTime(newDate, value.getHours(), value.getMinutes(), value.getSeconds())
        }
    ...
    handleDatePick(value) {
      if (this.selectionMode === 'day') {
        ...
        if (this.showTime) {
          const TimePickerRef = this.$refs.TimePicker
          if (TimePickerRef.selectedHour && TimePickerRef.selectedHour !== newDate.getHours()) {
            newDate.setHours(TimePickerRef.selectedHour)
          }
          if (
            TimePickerRef.selectedMinute &&
            TimePickerRef.selectedMinute !== newDate.getMinutes()
          ) {
            newDate.setMinutes(TimePickerRef.selectedMinute)
          }
        }
    ...
    confirm() {
      if (this.selectionMode === 'dates') {
        ...
        if (this.showTime) {
          const TimePickerRef = this.$refs.TimePicker
          if (TimePickerRef.selectedHour && TimePickerRef.selectedHour !== value.getHours()) {
            value.setHours(TimePickerRef.selectedHour)
          }
          if (TimePickerRef.selectedMinute && TimePickerRef.selectedMinute !== value.getMinutes()) {
            value.setMinutes(TimePickerRef.selectedMinute)
          }
        }
